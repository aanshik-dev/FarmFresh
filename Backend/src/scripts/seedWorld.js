 ...
 *
 * The seed is IDEMPOTENT: if the bot world is already seeded it skips.
 * Use `node src/scripts/seedWorld.js --reset` to wipe & rebuild (refuses to
 * wipe real user registrations unless `--force` is also passed).
 * All bot accounts share the password: password
 *
 * Run:   node src/scripts/seedWorld.js        (from Backend/)
 */
...
import seedCounters from "./seedCounters.js";
...
const DAY = 86400000;
// Any of these existing means the bot world is already seeded (real user
// registrations must NOT block reseeding).
const BOT_EMAILS = [
  "kedarnath.valley.farmers@gmail.com",
  "mandakini.organic@gmail.com",
  "farmers@gmail.com",
...
const USED_PHONES = new Set();
/** Deterministic 10-digit Indian mobile, guaranteed unique within the world. */
const genPhone = (seed) => {
  let s = seed;
  let p = mobile(s);
  while (USED_PHONES.has(p)) {
    s += 1;
    p = mobile(s);
  }
  USED_PHONES.add(p);
...
const buildFarmer = async (spec, ts) => {
  ...
  const profile = await saveAt(
    FarmerGroup,
    {
      ...
      name: spec.name,
      email: spec.email,
      phone: spec.phone || genPhone(500 + slugSeed(spec.slug)),
      profile: img(`farmer-${spec.slug}`),
      leadFarmer: spec.leadFarmer,
      farmerCount: spec.farmerCount,
      desc: spec.desc,
  ...
  for (const c of spec.crops) {
    const fc = await saveAt(
      ...
      {
        farmer: profile._id,
        crop: CROPS.get(c.name)._id,
        yield: c.yield,
        farmland: c.farmland,
        plantedDate: at(c.plantedDaysAgo, 9, 0),
        status: "ACTIVE",
      },
...
const buildCollective = async (spec, ts) => {
  ...
  const profile = await saveAt(
    Collective,
    {
      ...
      name: spec.name,
      email: spec.email,
      phone: spec.phone || genPhone(2000 + slugSeed(spec.slug)),
  ...
  for (const [i, d] of spec.drivers.entries()) {
    ...
    const driver = await saveAt(
      Driver,
      {
        collective: profile._id,
        driverId,
        profile: img(`driver-${spec.slug}-${i}`),
        name: d.name,
        phone: d.phone || genPhone(3000 + slugSeed(spec.slug) + i * 13),
        license: d.license,
        vehicleNumber: d.vehicleNumber,
        capacity: d.capacity,
        zones: collective.zones.map((z) => z._id),
        status: "AVAILABLE",
      },
...
const requestMembership = async ({ farmer, collective, crops, note, ts }) => {
  const membership = await saveAt(
    Membership,
    {
      farmer: farmer.profile._id,
      collective: collective.profile._id,
      status: "PENDING",
      note,
    },
    ts,
  ...
  for (const c of crops) {
    ...
    const deal = await saveAt(
      CropDeal,
      {
        ...
        crop: fc.doc._id,
        demandedPrice: c.demandedPrice,
        requestedQuantity: c.requestQty,
        agreedPrice: 0,
        status: "REQUESTED",
      },
      ts,
...
const acceptMembership = async ({
  ...
  if (accepted.length === 0) {
    // fully rejected — mirror collective service: membership goes REJECTED
    await touchAt(Membership, membership._id, { status: "REJECTED" }, ts);
  } else {
    const update = {
      status: "ACTIVE",
      memberSince: ts,
  ...
  await notify({
    ...
    sender: collective.user._id,
    ts: new Date(ts.getTime() + 120000),
  });
};

/** Resolve the partner User id from a CropDeal (via its Membership). */
const partnerOf = async (deal, which) => {
  const m = await Membership.findById(deal.membership)
    .select(`${which}`)
    .lean();
...
const setPickupDate = async ({ collective, deal, date, ts }) => {
  ...
  await touchAt(
    CropDeal,
    deal._id,
    { "schedule.expectedPickupDate": date },
    ts,
  );
  await notify({
    recipient: farmerId,
    ...
    data: { dealId: deal._id, expectedPickupDate: date },
    sender: collective.user._id,
    ts,
  });
};
...
const pickup = async ({
  ...
  let totalQuantity = 0;
  const farmerIds = [
    ...new Set(items.map((i) => i.farmer.profile._id.toString())),
  ];
  ...
  for (const i of itemDocs) {
    const item = await saveAt(
      ScheduleItem,
      {
        ...
        membership: i.membership._id,
        cropDeal: i.deal._id,
        cropName: i.cropName,
        cropCode: i.cropCode,
        plannedQuantity: i.plannedQty,
        collectedQuantity: i.collected,
  ...
  for (const i of itemDocs) {
    if (i.collected > 0) {
      perMembership.set(
        ...
        round2(
          (perMembership.get(i.membership._id.toString()) || 0) + i.amount,
        ),
      );
    }
    ...
    await CropDeal.updateOne(
      ...
      {
        $inc: {
          "schedule.totalCollected": i.collected,
          "schedule.pickupCount": i.collected > 0 ? 1 : 0,
        },
      },
      { timestamps: false },
    );
    // reduce farmer crop stock + increase collective inventory
    const fc = i.farmer.crops.find(
      (x) => x.doc._id.toString() === i.deal.crop.toString(),
    );
    if (fc) {
  ...
  for (const [fid, s] of perFarmer) {
    ...
    const fobj = WORLD.farmers.find((f) => f.profile._id.toString() === fid);
    if (fobj) fobj.pendingBalance = (fobj.pendingBalance || 0) + s.amount;
  }
  await touchAt(
    Schedule,
    schedule._id,
  ...
  for (const i of itemDocs) {
    if (i.collected > 0) {
      await notify({
        ...
        sender: collective.user._id,
        ts: new Date(completeAt.getTime() + 30000),
      });
    }
  }
  ...
  for (const p of pays) {
    const paidItems = itemDocs.filter(
      (i) =>
        i.farmer.profile._id.toString() === p.farmer.profile._id.toString() &&
        i.collected > 0,
    );
    ...
    const membership = WORLD.memberships.find(
      (m) =>
        m.farmer.toString() === p.farmer.profile._id.toString() &&
        m.collective.toString() === collective.profile._id.toString(),
    );
    const membershipDoc = await Membership.findById(membership._id);
    const balanceAfter = round2(
      Math.max(0, (membershipDoc.balance || 0) - totalPayment),
    );
    ...
    const tx = await saveAt(
      PaymentTransaction,
      {
        code: pCode,
        collective: collective.profile._id,
        farmerGroup: p.farmer.profile._id,
        membership: membership._id,
        schedule: schedule._id,
        items: paidItems.map((i) => i._item._id),
        amount: totalPayment,
        balanceAfter,
        method: p.method || "UPI",
        paymentProof: proof,
      ...
      { _id: p.farmer.profile._id },
      {
        $set: {
          pendingBalance: round2(
            Math.max(0, (p.farmer.pendingBalance || 0) - totalPayment),
          ),
        },
        $inc: { totalEarnings: totalPayment },
      },
    ...
    p.farmer.pendingBalance = round2(
      Math.max(0, (p.farmer.pendingBalance || 0) - totalPayment),
    );
    ...
    await notify({
      ...
      sender: collective.user._id,
      ts: new Date(p.ts.getTime() + 60000),
    });
  }
...
const reviewCollective = async ({
  ...
  const review = await saveAt(
    Review,
    { fid: farmer.profile._id, cid: collective.profile._id, comment, rating },
    ts,
  );
  WORLD.reviews.push(review);
  const all = await Review.find({ cid: collective.profile._id })
    .select("rating")
    .lean();
...
const seedWorld = async () => {
  ...
  const existing = await User.findOne({ username: { $in: BOT_EMAILS } });
  if (existing) {
    console.log(
      "ℹ️  Bot world already seeded, Use `npm run seed:world --reset --force` to rebuild it.",
    );
    return false;
  }
  ...
  if (missingCrops.length) {
    console.error(
      `❌ Master crop list is missing: ${missingCrops.join(", ")}. Run the server once (or seedData.js) first.`,
    );
    process.exit(1);
  }
  ...
  const mandakini = await buildCollective(
    {
      ...
      email: "mandakini.organic@gmail.com",
      manager: "Rajesh Negi",
      workers: 14,
      ...
      zones: [
        {
          name: "Kedar Zone",
          area: "Rudraprayag–Ukhimath belt",
          direction: "NE",
          desc: "Covers Rudraprayag, Guptkashi and Ukhimath blocks",
          color: "#10b981",
        },
        {
          name: "Alaknanda Zone",
          area: "Chamoli belt",
          direction: "E",
          desc: "Covers Gopeshwar and Joshimath blocks",
          color: "#3b82f6",
        },
      ],
      drivers: [
        {
          ...
          license: "UK07 20140045678",
          vehicleNumber: "UK07 PA 4582",
          capacity: 3000,
        },
        {
          name: "Mahendra Singh Rawat",
          license: "UK07 20170023456",
          vehicleNumber: "UK07 TA 1723",
          capacity: 2200,
        },
      ],
      crops: [
        ...
        { name: "Coriander", price: 45 },
        { name: "Fenugreek (Methi)", price: 40 },
        { name: "Garlic", price: 120 },
        { name: "Ginger", price: 90 },
        { name: "Wheat", price: 24 },
        { name: "Lentil (Masoor)", price: 85 },
        { name: "Green Gram (Moong)", price: 95 },
        { name: "Mustard", price: 55 },
      ],
    },
    at(340, 10, 20),
  );
  ...
  const garhwal = await buildCollective(
    {
      ...
      zones: [
        {
          name: "Rudra Zone",
          area: "Rudraprayag south",
          direction: "S",
          desc: "Farmers south of Rudraprayag town",
          color: "#f59e0b",
        },
        {
          name: "Chamoli Zone",
          area: "Chamoli east",
          direction: "E",
          desc: "Gopeshwar cluster",
          color: "#8b5cf6",
        },
      ],
      drivers: [
        {
          name: "Dinesh Chand Purohit",
          license: "UK07 20190012345",
          vehicleNumber: "UK07 UA 7810",
          capacity: 2600,
        },
        {
          name: "Sanjay Gusain",
          license: "UK07 20210056789",
          vehicleNumber: "UK07 UA 3345",
          capacity: 1800,
        },
      ],
      crops: [
        { name: "Tomato", price: 28 },
        { name: "Chilli", price: 60 },
        { name: "Capsicum", price: 70 },
        { name: "Cabbage", price: 17 },
        { name: "Onion", price: 24 },
        { name: "Potato", price: 21 },
        { name: "Carrot", price: 28 },
        { name: "Radish", price: 22 },
        { name: "Beetroot", price: 30 },
        { name: "Peas", price: 62 },
        { name: "Beans", price: 52 },
        { name: "Maize", price: 22 },
        { name: "Barley", price: 28 },
        { name: "Ragi (Finger Millet)", price: 45 },
        { name: "Black Gram (Urad)", price: 90 },
        { name: "Mustard", price: 54 },
      ],
    },
    at(325, 9, 40),
  );
  ...
  const himalayan = await buildCollective(
    {
      slug: "himalayan",
      name: "Himalayan Harvest Collective",
      email: "himalayan.harvest@gmail.com",
      manager: "Anil Thakur",
      workers: 11,
      ...
      zones: [
        {
          name: "Kullu Valley Zone",
          area: "Kullu–Manali belt",
          direction: "N",
          desc: "Apple and off-season veg growers of Kullu",
          color: "#ec4899",
        },
        {
          name: "Kangra Zone",
          area: "Palampur–Kangra belt",
          direction: "W",
          desc: "Tea, grain and ginger growers",
          color: "#14b8a6",
        },
      ],
      drivers: [
        {
          name: "Karan Pathania",
          license: "HP12 20150098765",
          vehicleNumber: "HP12 A 3344",
          capacity: 3200,
        },
      ],
      crops: [
        { name: "Apple", price: 145 },
        { name: "Kiwi", price: 160 },
        { name: "Strawberry", price: 180 },
        { name: "Plum", price: 115 },
        { name: "Peach", price: 120 },
        { name: "Capsicum", price: 72 },
        { name: "Cabbage", price: 19 },
        { name: "Peas", price: 68 },
        { name: "Ginger", price: 92 },
        { name: "Wheat", price: 25 },
        { name: "Barley", price: 29 },
      ],
    },
    at(310, 11, 5),
  );
  ...
  const kumaon = await buildCollective(
    {
      slug: "kumaon",
      name: "Kumaon Fresh Collective",
      email: "kumaon.fresh@gmail.com",
      manager: "Meena Joshi",
      workers: 8,
      ...
      drivers: [
        {
          name: "Prakash Bisht",
          license: "UK04 20180011223",
          vehicleNumber: "UK04 CA 2210",
          capacity: 2000,
        },
      ],
      crops: [
        ...
        { name: "Onion", price: 23 },
        { name: "Carrot", price: 27 },
        { name: "Beans", price: 52 },
        { name: "Cabbage", price: 16 },
        { name: "Tomato", price: 26 },
        { name: "Rice", price: 34 },
        { name: "Wheat", price: 23 },
        { name: "Lentil (Masoor)", price: 82 },
        { name: "Apple", price: 135 },
        { name: "Pear", price: 85 },
      ],
    },
    at(290, 14, 0),
  ...
  const devbhoomi = await buildCollective(
    {
      slug: "devbhoomi",
      name: "Devbhoomi Farmer Market",
      email: "devbhoomi.market@gmail.com",
      manager: "Vijay Rawat",
      workers: 12,
      ...
      zones: [
        {
          name: "Chamoli Zone",
          area: "Gopeshwar–Joshimath",
          direction: "NE",
          desc: "Highland pulses & spices",
          color: "#a855f7",
        },
        {
          name: "Rudra Zone",
          area: "Guptkashi cluster",
          direction: "SE",
          desc: "Guptkashi farmers",
          color: "#22c55e",
        },
      ],
      drivers: [
        {
          name: "Ramesh Kandari",
          license: "UK07 20160033445",
          vehicleNumber: "UK07 TA 9001",
          capacity: 2400,
        },
        {
          name: "Naveen Rana",
          license: "UK07 20200077889",
          vehicleNumber: "UK07 PA 6621",
          capacity: 1600,
        },
      ],
      crops: [
        { name: "Turmeric", price: 110 },
        { name: "Garlic", price: 118 },
        { name: "Ginger", price: 88 },
        { name: "Lentil (Masoor)", price: 83 },
        { name: "Green Gram (Moong)", price: 93 },
        { name: "Ragi (Finger Millet)", price: 44 },
        { name: "Maize", price: 21 },
  ...
  const testHub = await buildCollective(
    {
      slug: "testhub",
      name: "Collective Test Hub",
      email: "collective@gmail.com",
      manager: "Test Manager",
      workers: 4,
      desc: "Test collective used by the development team to demo the app end-to-end.",
      address: {
        locality: "Balawala",
        area: "Shyampur",
        town: "Rishikesh",
      ...
      coord: { lat: 30.09, long: 78.25 },
      zones: [
        {
          name: "Test Zone",
          area: "Rishikesh–Rudraprayag belt",
          direction: "NE",
          desc: "Demo pickup route",
          color: "#f59e0b",
        },
      ],
      drivers: [
        {
          name: "Test Driver",
          license: "UK07 20240000001",
          vehicleNumber: "UK07 TA 0001",
          capacity: 2000,
        },
      ],
      crops: [
        { name: "Potato", price: 22 },
        { name: "Tomato", price: 27 },
        { name: "Cabbage", price: 16 },
        { name: "Carrot", price: 28 },
      ],
    },
  ...
  const kedarnath = await buildFarmer(
    {
      slug: "kedarnath",
      name: "Kedarnath Valley Farmers",
      leadFarmer: "Devendra Prasad Semwal",
      email: "kedarnath.valley.farmers@gmail.com",
      farmerCount: 18,
      desc: "Organic grower group from the Mandakini valley — vegetables, pulses and temperate fruit since 2019.",
      address: {
        locality: "Village Tilwara",
  ...
  const ukhimathGroup = await buildFarmer(
    {
      slug: "ukhimath",
      name: "Ukhimath Organic Group",
      leadFarmer: "Ganga Devi Negi",
      email: "ukhimath.organic.group@gmail.com",
      farmerCount: 12,
      desc: "Women-led organic farming group in the Ukhimath block of Rudraprayag.",
      address: {
        locality: "Village Kalimath",
        area: "Ukhimath Block",
        town: "Ukhimath",
        district: "Rudraprayag",
        state: "Uttarakhand",
  ...
  const guptkashiGroup = await buildFarmer(
    {
      slug: "guptkashi",
      name: "Guptkashi Growers",
      leadFarmer: "Mohan Chandra Gairola",
      email: "guptkashi.growers@gmail.com",
      farmerCount: 9,
      desc: "Mixed vegetable and pulse growers around Guptkashi on the Kedarnath highway.",
      address: {
        locality: "Village Sitapur",
        area: "Guptkashi",
        town: "Guptkashi",
        district: "Rudraprayag",
        state: "Uttarakhand",
  ...
  const gopeshwarGroup = await buildFarmer(
    {
      slug: "gopeshwar",
      name: "Gopeshwar Green Collective",
      leadFarmer: "Chandra Singh Panwar",
      email: "gopeshwar.green@gmail.com",
      farmerCount: 15,
      desc: "Chamoli-based grower group producing pulses, maize and hill vegetables.",
      address: {
        locality: "Narayan Bagar",
        area: "Gopeshwar",
        town: "Gopeshwar",
        district: "Chamoli",
        state: "Uttarakhand",
  ...
  const joshimathGroup = await buildFarmer(
    {
      slug: "joshimath",
      name: "Joshimath Hills Farmers",
      leadFarmer: "Kailash Prasad Kotiyal",
      email: "joshimath.hills@gmail.com",
      farmerCount: 8,
      desc: "High-altitude vegetable growers from Joshimath, Chamoli.",
      address: {
        locality: "Village Auli",
        area: "Joshimath",
        town: "Joshimath",
        district: "Chamoli",
        state: "Uttarakhand",
  ...
  const bageshwarGroup = await buildFarmer(
    {
      slug: "bageshwar",
      name: "Bageshwar Organic Producers",
      leadFarmer: "Laxmi Devi Bhatt",
      email: "bageshwar.organic@gmail.com",
      farmerCount: 10,
      desc: "Kumaon organic producers from the Saryu valley, Bageshwar.",
      address: {
        locality: "Village Kanda",
        area: "Garur",
        town: "Bageshwar",
        district: "Bageshwar",
        state: "Uttarakhand",
  ...
  const almoraGroup = await buildFarmer(
    {
      slug: "almora",
      name: "Almora Apple Growers",
      leadFarmer: "Harish Chandra Joshi",
      email: "almora.apple.growers@gmail.com",
      farmerCount: 7,
      desc: "Temperate fruit growers of Almora's orchards.",
      address: {
        locality: "Village Lamgarha",
        area: "Hawalbagh",
        town: "Almora",
        district: "Almora",
        state: "Uttarakhand",
  ...
  const kulluGroup = await buildFarmer(
    {
      slug: "kullu",
      name: "Kullu Valley Farmers",
      leadFarmer: "Rakesh Kumar Thakur",
      email: "kullu.valley.farmers@gmail.com",
      farmerCount: 20,
      desc: "Apple orchards and off-season vegetable growers of the Kullu valley, HP.",
      address: {
        locality: "Village Raison",
        area: "Kullu",
        town: "Kullu",
        district: "Kullu",
        state: "Himachal Pradesh",
  ...
  const manaliGroup = await buildFarmer(
    {
      slug: "manali",
      name: "Manali Apple & Veg Group",
      leadFarmer: "Sonam Chand Sharma",
      email: "manali.apple.veg@gmail.com",
      farmerCount: 11,
      desc: "High-altitude growers from Manali and Sethan villages.",
      address: {
        locality: "Village Naggar",
        area: "Manali",
        town: "Manali",
        district: "Kullu",
        state: "Himachal Pradesh",
  ...
  const kangraGroup = await buildFarmer(
    {
      slug: "kangra",
      name: "Kangra Tea & Grain Growers",
      leadFarmer: "Sudesh Kumari",
      email: "kangra.grain.growers@gmail.com",
      farmerCount: 13,
      desc: "Palampur-area farmers growing grains, ginger and spices.",
      address: {
        locality: "Village Galot",
        area: "Palampur",
        town: "Palampur",
        district: "Kangra",
        state: "Himachal Pradesh",
  ...
  const solanGroup = await buildFarmer(
    {
      slug: "solan",
      name: "Solan Vegetable Farmers",
      leadFarmer: "Om Prakash Verma",
      email: "solan.veg.farmers@gmail.com",
      farmerCount: 16,
      desc: "Off-season vegetable producers from the Solan valley.",
      address: {
        locality: "Village Kandaghat",
        area: "Solan",
        town: "Solan",
        district: "Solan",
        state: "Himachal Pradesh",
  ...
  const testFarmer = await buildFarmer(
    {
      slug: "testfarmer",
      name: "Test Farmer Group",
      leadFarmer: "Test Lead Farmer",
      email: "farmers@gmail.com",
      farmerCount: 5,
      desc: "Test farmer group used by the development team to demo the app end-to-end.",
      address: {
        locality: "Village Testar",
        area: "Mandakini Valley",
        town: "Rudraprayag",
        district: "Rudraprayag",
        state: "Uttarakhand",
  ...
  // ── Story A: mature, 3 pickups (2 paid, 1 pending) — Kedarnath x Mandakini ──
  {
    ...
    await acceptMembership({
      ...
      rejected: [
        {
          deal: req.deals[2].deal,
          reason:
            "Pea demand is already full for this season. Try again in the next cycle.",
        },
        {
          deal: req.deals[3].deal,
          reason: "Carrot quality spec not met in the last sample check.",
        },
      ],
      zone,
      route: "Tilwara → Rudraprayag → NH-7 → Rishikesh",
      distance: 165,
      estTime: 210,
    });
    const dCabbage = req.deals[0].deal,
      dCauli = req.deals[1].deal;
    const cabbage = F.crops[0],
      cauli = F.crops[1];
    ...
    await grow({
      ...
      deal: dCabbage,
      stage: "GROWING",
      message: "Heads forming well, top dressing done.",
      ts: at(262, 9, 40),
      imgSeed: "cabbage-grow-1",
    });
    await grow({
      farmer: F,
      deal: dCabbage,
      stage: "MATURE",
      message: "Heads firm, harvest begins next week.",
      ts: at(232, 10, 15),
      imgSeed: "cabbage-mature-1",
    });
    await grow({
      farmer: F,
      deal: dCabbage,
      stage: "READY",
      message: "2400 kg ready for pickup.",
      ts: at(205, 16, 30),
      imgSeed: "cabbage-ready-1",
    });
    await grow({
      farmer: F,
      deal: dCauli,
      stage: "GROWING",
      message: "Curds developing well.",
      ts: at(255, 9, 0),
      imgSeed: "cauli-grow-1",
    });
    await grow({
      farmer: F,
      deal: dCauli,
      stage: "MATURE",
      message: "Curds compact and white.",
      ts: at(225, 11, 20),
      imgSeed: "cauli-mature-1",
    });
    await grow({
      farmer: F,
      deal: dCauli,
      stage: "READY",
      message: "1600 kg ready, first harvest done.",
      ts: at(198, 15, 45),
      imgSeed: "cauli-ready-1",
    });
    ...
    await grow({
      ...
      deal: dCabbage,
      stage: "READY",
      message: "Confirmed — harvest complete, crates packed.",
      ts: at(197, 18, 10),
      imgSeed: "cabbage-qr-1",
    });
    ...
    await pickup({
      ...
      zone,
      driver,
      createdAt: at(188, 14, 0),
      pickupDate: at(185, 9, 0),
      time: "09:00",
      notes: "First scheduled pickup of the season.",
      startAt: at(185, 8, 15),
      completeAt: at(185, 14, 20),
      items: [
        {
          farmer: F,
          deal: dCabbage,
          cropName: "Cabbage",
          cropCode: CROPS.get("Cabbage").code,
          agreedPrice: 18,
          plannedQty: 2400,
          collectQty: 2350,
          membership: req.membership,
          remark: "2 crates lighter than planned",
        },
        {
          farmer: F,
          deal: dCauli,
          cropName: "Cauliflower",
          cropCode: CROPS.get("Cauliflower").code,
          agreedPrice: 25,
          plannedQty: 1600,
          collectQty: 1580,
          membership: req.membership,
      ...
      pays: [
        {
          ...
          utr: "UTR-88451290",
          proofSeed: "pay-1",
          remarks: "Settlement for pickup 1 — cabbage & cauliflower",
          ts: at(180, 15, 30),
        },
      ],
    });
    ...
    await grow({
      ...
      deal: dCabbage,
      stage: "READY",
      message: "Second batch ready — 2200 kg.",
      ts: at(172, 10, 10),
      imgSeed: "cabbage-ready-2",
    });
    await grow({
      farmer: F,
      deal: dCauli,
      stage: "READY",
      message: "Second flush ready — 1500 kg.",
      ts: at(168, 9, 30),
      imgSeed: "cauli-ready-2",
    });
    ...
    await grow({
      ...
      deal: dCabbage,
      stage: "READY",
      message: "Confirmed ready.",
      ts: at(163, 16, 0),
      imgSeed: "cabbage-qr-2",
    });
    ...
    await pickup({
      ...
      zone,
      driver,
      createdAt: at(162, 13, 0),
      pickupDate: at(150, 9, 30),
      time: "09:30",
      notes: "Mid-season pickup.",
      startAt: at(150, 8, 45),
      completeAt: at(150, 15, 10),
      items: [
        {
          farmer: F,
          deal: dCabbage,
        ...
        },
        {
          ...
          collectQty: 1460,
          membership: req.membership,
          remark: "Road rain damage — 40kg rejected",
        },
      ],
      pays: [
        {
          ...
          utr: "UTR-90234156",
          proofSeed: "pay-2",
          remarks: "Settlement for pickup 2",
          ts: at(145, 12, 0),
        },
      ],
    });
    ...
    await grow({
      ...
      deal: dCabbage,
      stage: "READY",
      message: "Late season batch — 2000 kg ready.",
      ts: at(40, 9, 0),
      imgSeed: "cabbage-ready-3",
    });
    await grow({
      farmer: F,
      deal: dCauli,
      stage: "READY",
      message: "1800 kg cauliflower ready.",
      ts: at(36, 11, 20),
      imgSeed: "cauli-ready-3",
    });
    ...
    await reviewCollective({
      ...
      rating: 5,
      comment:
        "Very professional pickup coordination. Payments are prompt and the driver is always on time.",
      ts: at(178, 11, 0),
    });
  }
  ...
  // ── Story B: partial accept, one paid pickup — Guptkashi x Mandakini ───────
  {
    ...
    await acceptMembership({
      ...
      zone,
      route: "Guptkashi → Rudraprayag → Rishikesh",
      distance: 185,
      estTime: 240,
    });
    const dPotato = req.deals[0].deal,
      dTomato = req.deals[1].deal;

    await grow({
      farmer: F,
      deal: dPotato,
      stage: "GROWING",
      message: "Plants healthy, earthing done.",
      ts: at(210, 9, 20),
      imgSeed: "potato-grow-1",
    });
    await grow({
      farmer: F,
      deal: dPotato,
      stage: "MATURE",
      message: "Tubers sizing well.",
      ts: at(175, 10, 40),
      imgSeed: "potato-mature-1",
    });
    await grow({
      farmer: F,
      deal: dPotato,
      stage: "READY",
      message: "2200 kg potatoes ready.",
      ts: at(150, 15, 0),
      imgSeed: "potato-ready-1",
    });
    await grow({
      farmer: F,
      deal: dTomato,
      stage: "GROWING",
      message: "Flowering started.",
      ts: at(205, 9, 0),
      imgSeed: "tomato-grow-1",
    });
    await grow({
      farmer: F,
      deal: dTomato,
      stage: "MATURE",
      message: "First pick done.",
      ts: at(168, 12, 30),
      imgSeed: "tomato-mature-1",
    });
    await grow({
      farmer: F,
      deal: dTomato,
      stage: "READY",
      message: "1500 kg ready.",
      ts: at(143, 16, 20),
      imgSeed: "tomato-ready-1",
    });
    ...
    await pickup({
      ...
      zone,
      driver,
      createdAt: at(140, 12, 30),
      pickupDate: at(128, 10, 0),
      time: "10:00",
      notes: "Potato + tomato pickup.",
      startAt: at(128, 9, 0),
      completeAt: at(128, 16, 45),
      items: [
        {
          farmer: F,
          deal: dPotato,
          cropName: "Potato",
          cropCode: CROPS.get("Potato").code,
          agreedPrice: 22,
          plannedQty: 2200,
          collectQty: 2100,
          membership: req.membership,
        ...
        },
        {
          ...
          deal: dTomato,
          cropName: "Tomato",
          cropCode: CROPS.get("Tomato").code,
          agreedPrice: 28,
          plannedQty: 1500,
          collectQty: 1420,
          membership: req.membership,
          remark: "Overripe batch rejected at gate",
        },
      ],
      pays: [
        {
          ...
          utr: "UTR-91203445",
          proofSeed: "pay-b1",
          remarks: "Settlement for potato & tomato pickup",
          ts: at(122, 14, 15),
        },
      ],
    });
    ...
    await reviewCollective({
      ...
      rating: 4,
      comment:
        "Good rates, timely payments. Delivery points are a bit far for small groups.",
      ts: at(120, 10, 30),
    });
  }
  ...
  // ── Story D: fully rejected — Bageshwar x Kumaon ────────────────────────────
  {
    ...
    await acceptMembership({
      ...
      rejected: [
        {
          deal: req.deals[0].deal,
          reason: "Rice procurement quota is closed for this season.",
        },
        {
          deal: req.deals[1].deal,
          reason: "Wheat grades did not match our moisture spec.",
        },
      ],
      zone: C.zones[0],
      route: "",
  ...
  // ── Story C: fresh pending request (live state) — Bageshwar re-applies to Kumaon ──
  {
    const F = bageshwarGroup,
      C = kumaon;
    // Story D's membership was fully REJECTED — the real service reuses it (PENDING again)
    const membership = WORLD.memberships.find(
      (m) =>
        m.farmer.toString() === F.profile._id.toString() &&
    ...
    const upsertDeal = async (name, demandedPrice, requestQty) => {
      const fc = F.crops.find((x) => x.name === name);
      const old = WORLD.deals.find(
        (d) =>
          d.spec &&
          d.spec.name === name &&
          d.deal.membership.toString() === membership._id.toString(),
      );
      if (old) {
        await touchAt(
          CropDeal,
        ...
        WORLD.deals.push({ crop: fc.doc, deal, spec: { name, requestQty } });
      }
    };
    await upsertDeal("Lentil (Masoor)", 78, 950);
    await upsertDeal("Onion", 22, 1300);
    await upsertDeal("Wheat", 22, 2400);
    await notify({
      recipient: C.user._id,
      recipientRole: "COLLECTIVE",
      type: "REQUEST",
      title: "New Membership Request",
      body: `${F.profile.name} has sent a new membership request for 3 crop(s).`,
      data: { farmerId: F.profile._id, requestCount: 3 },
      sender: F.user._id,
      ts: new Date(at(4, 10, 45).getTime() + 60000),
    });
  }

  // ── Story E: farmer cancelled own request — Joshimath x Mandakini ───────────
  {
    const F = joshimathGroup,
      C = mandakini;
    const req = await requestMembership({
      farmer: F,
      collective: C,
  ...
  // ── Story F: approved then terminated by farmer — Ukhimath x Mandakini ──────
  {
    ...
    await acceptMembership({
      ...
      rejected: [
        {
          deal: req.deals[2].deal,
          reason: "Spinach volume too small for the route.",
        },
      ],
      zone,
      route: "Kalimath → Ukhimath → Rishikesh",
      distance: 205,
      estTime: 260,
    });
    const dCabbage = req.deals[0].deal,
      dBeans = req.deals[1].deal;

    await grow({
      farmer: F,
      deal: dCabbage,
      stage: "GROWING",
      message: "Good head formation.",
      ts: at(200, 9, 10),
      imgSeed: "uk-cabbage-grow",
    });
    await grow({
      farmer: F,
      deal: dCabbage,
      ...
      ts: at(170, 14, 0),
      imgSeed: "uk-cabbage-ready",
    });
    await grow({
      farmer: F,
      deal: dBeans,
      stage: "GROWING",
      message: "Vines flowering.",
      ts: at(195, 10, 0),
      imgSeed: "uk-beans-grow",
    });
    await grow({
      farmer: F,
      deal: dBeans,
    ...
    await pickup({
      ...
      zone,
      driver,
      createdAt: at(160, 11, 0),
      pickupDate: at(148, 9, 30),
      time: "09:30",
      notes: "Ukhimath route pickup.",
      startAt: at(148, 8, 45),
      completeAt: at(148, 17, 20),
      items: [
        {
          farmer: F,
          deal: dCabbage,
      ...
      pays: [
        {
          ...
          method: "CASH",
          proofSeed: "pay-f1",
          remarks: "Cash settlement at pickup point",
          ts: at(144, 11, 30),
        },
      ],
    });
    ...
    await touchAt(
      ...
      dCabbage._id,
      {
        status: "F_TERMINATE",
        terminationReason:
          "Cabbage season over — moving to spinach crop for next cycle.",
      },
      at(90, 16, 0),
    );
  }
  ...
  // ── Story G: completed pickup, fully unpaid (collective owes) — Gopeshwar x Devbhoomi ──
  {
    ...
    await acceptMembership({
      ...
      rejected: [],
      zone,
      route: "Gopeshwar → Chamoli → Haridwar",
      distance: 210,
      estTime: 280,
    });
    const dMaize = req.deals[0].deal,
      dRagi = req.deals[1].deal;
    await grow({
      farmer: F,
      deal: dMaize,
      stage: "GROWING",
      message: "Cobs forming.",
      ts: at(95, 9, 0),
      imgSeed: "maize-grow-1",
    });
    await grow({
      farmer: F,
      deal: dMaize,
      stage: "READY",
      message: "2600 kg maize ready.",
      ts: at(60, 10, 30),
      imgSeed: "maize-ready-1",
    });
    await grow({
      farmer: F,
      deal: dRagi,
      stage: "GROWING",
      message: "Grains filling.",
      ts: at(90, 11, 0),
      imgSeed: "ragi-grow-1",
    });
    await grow({
      ...
      deal: dRagi,
      stage: "READY",
      message: "1200 kg ragi ready.",
      ts: at(55, 14, 0),
      imgSeed: "ragi-ready-1",
    });
    ...
    await pickup({
      ...
      zone,
      driver,
      createdAt: at(52, 10, 0),
      pickupDate: at(40, 9, 0),
      time: "09:00",
      notes: "Pulse & grain pickup — payment pending.",
      startAt: at(40, 8, 30),
      completeAt: at(40, 15, 40),
      items: [
        {
          farmer: F,
          deal: dMaize,
          cropName: "Maize",
          cropCode: CROPS.get("Maize").code,
          agreedPrice: 22,
          plannedQty: 2600,
          collectQty: 2500,
          membership: req.membership,
        ...
        },
        {
          ...
          cropCode: CROPS.get("Ragi (Finger Millet)").code,
          agreedPrice: 45,
          plannedQty: 1200,
          collectQty: 1150,
          membership: req.membership,
    ...
    await reviewCollective({
      ...
      rating: 3,
      comment:
        "Pickup went fine but the payment is delayed. Following up every week.",
      ts: at(38, 12, 0),
    });
  }
  ...
  // ── Story H1: upcoming scheduled pickup (+5d) — Kedarnath x Mandakini (new crop) ──
  {
    ...
    const dPotato = await saveAt(
      CropDeal,
      {
        ...
        crop: fc.doc._id,
        demandedPrice: 21,
        requestedQuantity: 2800,
        agreedPrice: 22,
        status: "APPROVED",
        approvalDate: at(19, 10, 0),
        growth: {
          stage: "READY",
          expectedQuantity: 2800,
          queryStatus: "CLOSED",
          lastUpdated: at(6, 9, 30),
          images: [img("potato-upcoming")],
          message: "Harvest complete, 2800 kg packed.",
        },
        schedule: {
          expectedPickupDate: at(-5, 9, 0),
          collectedQuantity: 0,
          totalCollected: 0,
          activeSchedule: null,
          lastPickupDate: null,
    ...
    const schedule = await saveAt(
      ...
      {
        code,
        collective: C.profile._id,
        driver: driver._id,
        zone: zone._id,
        pickupDate: at(-5, 9, 0),
        time: "09:00",
        status: "SCHEDULED",
        totalAmount: round2(2800 * 22),
        totalQuantity: 2800,
        paidAmount: 0,
        farmerCount: 1,
        itemCount: 1,
        notes: "Potato pickup — Tilwara crates.",
      },
      at(7, 13, 0),
    );
    await saveAt(
      ScheduleItem,
      {
        schedule: schedule._id,
        collective: C.profile._id,
        farmerGroup: F.profile._id,
        membership: membership._id,
        cropDeal: dPotato._id,
        cropName: "Potato",
        cropCode: CROPS.get("Potato").code,
        plannedQuantity: 2800,
        collectedQuantity: 2800,
        agreedPrice: 22,
        totalAmount: round2(2800 * 22),
        status: "PENDING",
        paymentStatus: "PENDING",
  ...
  // ── Story H2: upcoming pickup (+9d) + postponed history — Kullu x Himalayan ──
  {
    ...
    await acceptMembership({
      ...
      rejected: [],
      zone,
      route: "Raison → Kullu → NH-3 → Shimla",
      distance: 220,
      estTime: 290,
    });
    const dApple = req.deals[0].deal,
      dKiwi = req.deals[1].deal;
    await grow({
      farmer: F,
      deal: dApple,
      stage: "GROWING",
      message: "Fruit set good this year.",
      ts: at(110, 9, 30),
      imgSeed: "apple-grow-1",
    });
    await grow({
      farmer: F,
      deal: dApple,
      stage: "MATURE",
      message: "Colour developing nicely.",
      ts: at(70, 10, 0),
      imgSeed: "apple-mature-1",
    });
    await grow({
      farmer: F,
      deal: dApple,
      stage: "READY",
      message: "First-grade apples ready for grading.",
      ts: at(30, 15, 20),
      imgSeed: "apple-ready-1",
    });
    await grow({
      farmer: F,
      deal: dKiwi,
      stage: "GROWING",
      message: "Vines heavy with fruit.",
      ts: at(100, 11, 30),
      imgSeed: "kiwi-grow-1",
    });
    await grow({
      ...
      deal: dKiwi,
      stage: "MATURE",
      message: "Kiwis softening — harvest in 2 weeks.",
      ts: at(60, 9, 45),
      imgSeed: "kiwi-mature-1",
    });
    await grow({
      farmer: F,
      deal: dKiwi,
      stage: "READY",
      message: "1100 kg kiwi ready.",
      ts: at(25, 16, 10),
      imgSeed: "kiwi-ready-1",
    });
    ...
    await pickup({
      ...
      pickupDate: at(85, 10, 0),
      time: "10:00",
      notes: "Early apple consignment.",
      startAt: at(85, 9, 15),
      completeAt: at(85, 16, 30),
      items: [
        {
          farmer: F,
          deal: dApple,
          cropName: "Apple",
          cropCode: CROPS.get("Apple").code,
          agreedPrice: 145,
          plannedQty: 2000,
          collectQty: 1900,
          membership: req.membership,
      ...
      pays: [
        {
          ...
          utr: "UTR-88990011",
          proofSeed: "pay-h1",
          remarks: "Early apple consignment settlement",
          ts: at(80, 13, 0),
        },
      ],
    });
    ...
    const schedule = await saveAt(
      Schedule,
      {
        ...
        time: "09:00",
        status: "POSTPONED",
        totalAmount: round2(1900 * 145 + 1100 * 160),
        totalQuantity: 3000,
        paidAmount: 0,
        farmerCount: 1,
        itemCount: 2,
        notes: "Main apple + kiwi dispatch.",
        postponeHistory: [
          {
            from: at(-2, 9, 0),
            to: at(-9, 9, 0),
    ...
    for (const it of items2) {
      await saveAt(
        ScheduleItem,
        {
          ...
          cropDeal: it.deal._id,
          cropName: it.name,
          cropCode: CROPS.get(it.name).code,
          plannedQuantity: it.qty,
          collectedQuantity: it.qty,
          agreedPrice: it.price,
          totalAmount: round2(it.qty * it.price),
          status: "PENDING",
          paymentStatus: "PENDING",
        },
        at(5, 11, 0),
      ...
      await touchAt(
        ...
        {
          "schedule.activeSchedule": schedule._id,
          "schedule.collectedQuantity": it.qty,
        },
        at(5, 11, 0),
      );
    }
    ...
    await reviewCollective({
      ...
      rating: 5,
      comment:
        "Best apple rates in the valley. The grading feedback has improved our packing a lot.",
      ts: at(78, 12, 30),
    });
  }
  ...
  // ── Story I: LIVE pickup today (IN_PROGRESS) — Gopeshwar + Joshimath x Garhwal ──
  {
    ...
    await acceptMembership({
      ...
      rejected: [],
      zone,
      route: "Joshimath → Chamoli → Rishikesh",
      distance: 235,
      estTime: 310,
    });
    await grow({
      farmer: F2,
      deal: req2.deals[0].deal,
      stage: "GROWING",
      message: "Heads firming.",
      ts: at(8, 9, 0),
      imgSeed: "jo-cabbage-grow",
    });
    await grow({
      farmer: F2,
      deal: req2.deals[0].deal,
      stage: "READY",
      message: "1300 kg ready.",
      ts: at(2, 15, 30),
      imgSeed: "jo-cabbage-ready",
    });
    await grow({
      farmer: F2,
      deal: req2.deals[1].deal,
      stage: "GROWING",
      message: "Pods filling.",
      ts: at(8, 9, 20),
      imgSeed: "jo-peas-grow",
    });
    await grow({
      farmer: F2,
      deal: req2.deals[1].deal,
      stage: "READY",
      message: "900 kg peas ready.",
      ts: at(2, 16, 0),
      imgSeed: "jo-peas-ready",
    });
    ...
    await acceptMembership({
      ...
      rejected: [],
      zone,
      route: "Gopeshwar → Chamoli → Rishikesh",
      distance: 210,
      estTime: 280,
    });
    await grow({
      farmer: F1,
      deal: req1.deals[0].deal,
      stage: "READY",
      message: "750 kg carrots ready.",
      ts: at(1, 10, 0),
      imgSeed: "carrot-ready-live",
    });
    await grow({
      farmer: F1,
      deal: req1.deals[1].deal,
      stage: "READY",
      message: "800 kg beans ready.",
      ts: at(1, 10, 20),
      imgSeed: "beans-ready-live",
    });
    ...
    const schedule = await saveAt(
      Schedule,
      {
        ...
        farmerCount: 2,
        itemCount: 4,
        notes: "Live Chamoli route pickup.",
      },
      at(3, 12, 0),
    );
    const liveItems = [
    ...
    for (const it of liveItems) {
      await saveAt(
        ScheduleItem,
        {
          schedule: schedule._id,
          collective: C.profile._id,
          farmerGroup: it.farmer.profile._id,
          membership: it.membership._id,
          cropDeal: it.deal._id,
          cropName: it.name,
          cropCode: CROPS.get(it.name).code,
          plannedQuantity: it.qty,
    ...
    started.setHours(8, 15, 0, 0);
    await touchAt(
      Schedule,
      schedule._id,
      { status: "IN_PROGRESS", startedAt: started },
      started,
    );
    await touchAt(Driver, driver._id, { status: "ONROUTE" }, started);
    await notify({
      recipient: F1.user._id,
      recipientRole: "FARMER_GROUP",
      type: "PICKUP",
      title: `Pickup In Progress · ${code}`,
      body: "The driver is on the way for today's collection. Please keep the harvest ready.",
  ...
  // ── Story J: cancelled pickup (weather) — Almora x Kumaon ────────────────────
  {
    ...
    await acceptMembership({
      ...
      rejected: [],
      zone,
      route: "Almora → Bageshwar → Haldwani",
      distance: 150,
      estTime: 200,
    });
    await grow({
      ...
      deal: req.deals[0].deal,
      stage: "GROWING",
      message: "Orchard looking good.",
      ts: at(55, 9, 0),
      imgSeed: "almora-apple-grow",
    });
    await grow({
      farmer: F,
      deal: req.deals[0].deal,
      stage: "MATURE",
      message: "Apples colouring.",
      ts: at(35, 11, 0),
      imgSeed: "almora-apple-mature",
    });
    await grow({
      farmer: F,
      deal: req.deals[0].deal,
      stage: "READY",
      message: "3200 kg apples ready.",
      ts: at(18, 15, 0),
      imgSeed: "almora-apple-ready",
    });
    ...
    const schedule = await saveAt(
      Schedule,
      {
        ...
        driver: driver._id,
        zone: zone._id,
        pickupDate: at(10, 9, 0),
        time: "09:00",
        status: "SCHEDULED",
        totalAmount: round2(3200 * 140),
        totalQuantity: 3200,
        paidAmount: 0,
        farmerCount: 1,
        itemCount: 1,
        notes: "Almora apple pickup.",
      },
      at(15, 10, 0),
    );
    await saveAt(
      ScheduleItem,
      {
        ...
        farmerGroup: F.profile._id,
        membership: req.membership._id,
        cropDeal: req.deals[0].deal._id,
        cropName: "Apple",
        cropCode: CROPS.get("Apple").code,
        plannedQuantity: 3200,
        collectedQuantity: 3200,
        agreedPrice: 140,
        totalAmount: round2(3200 * 140),
        status: "PENDING",
        paymentStatus: "PENDING",
    ...
    await touchAt(
      ...
      {
        "schedule.activeSchedule": schedule._id,
        "schedule.collectedQuantity": 3200,
      },
      at(15, 10, 0),
    );
    ...
    await touchAt(
      ...
      schedule._id,
      {
        status: "CANCELLED",
        cancelledAt,
        cancellationReason: "Landslide blocked the Almora–Haldwani road.",
      },
      cancelledAt,
    );
    await touchAt(
      ...
      rejected: [
        {
          deal: req.deals[2].deal,
          reason:
            "Ginger volumes from Kangra are already contracted this cycle.",
        },
      ],
      zone,
      route: "Palampur → Kangra → Shimla",
      distance: 195,
      estTime: 260,
    });
    await grow({
      ...
      deal: req.deals[0].deal,
      stage: "GROWING",
      message: "Wheat tillering well.",
      ts: at(60, 9, 30),
      imgSeed: "kangra-wheat-grow",
    });
    await grow({
      farmer: F,
      deal: req.deals[1].deal,
      stage: "GROWING",
      message: "Barley healthy stand.",
      ts: at(58, 10, 0),
      imgSeed: "kangra-barley-grow",
    });
    await grow({
      farmer: F,
      deal: req.deals[0].deal,
      stage: "MATURE",
      message: "Heads filling.",
      ts: at(20, 11, 0),
      imgSeed: "kangra-wheat-mature",
    });
  }
  ...
  // ── Story L: single-crop small partner, one completed unpaid pickup — Solan x Garhwal ──
  {
    ...
    await acceptMembership({
      ...
      rejected: [],
      zone,
      route: "Kandaghat → Solan → Rishikesh",
      distance: 190,
      estTime: 250,
    });
    const dTomato = req.deals[0].deal,
      dChilli = req.deals[1].deal;
    await grow({
      farmer: F,
      deal: dTomato,
      stage: "GROWING",
      message: "Vines staked.",
      ts: at(35, 9, 0),
      imgSeed: "solan-tomato-grow",
    });
    await grow({
      farmer: F,
      deal: dTomato,
      stage: "READY",
      message: "2600 kg tomatoes ready.",
      ts: at(12, 14, 30),
      imgSeed: "solan-tomato-ready",
    });
    await grow({
      farmer: F,
      deal: dChilli,
      stage: "GROWING",
      message: "Flowering well.",
      ts: at(33, 10, 0),
      imgSeed: "solan-chilli-grow",
    });
    await grow({
      farmer: F,
      deal: dChilli,
      stage: "READY",
      message: "900 kg chillies ready.",
      ts: at(10, 15, 0),
      imgSeed: "solan-chilli-ready",
    });
    ...
    await pickup({
      ...
      zone,
      driver,
      createdAt: at(8, 12, 30),
      pickupDate: at(6, 9, 30),
      time: "09:30",
      notes: "Solan vegetable pickup.",
      startAt: at(6, 8, 45),
      completeAt: at(6, 14, 20),
      items: [
        {
          farmer: F,
          deal: dTomato,
        ...
        },
        {
          ...
          deal: dChilli,
          cropName: "Chilli",
          cropCode: CROPS.get("Chilli").code,
          agreedPrice: 60,
          plannedQty: 900,
          collectQty: 860,
          membership: req.membership,
  ...
  // ── Story M: Ukhimath second collective (Garhwal) — completed pickup, payment pending ──
  {
    const F = ukhimathGroup,
      C = garhwal;
    const zone = C.zones[0],
    ...
    const req = await requestMembership({
      ...
      collective: C,
      ts: at(50, 9, 20),
      note: "Cabbage and beans for the spring cycle.",
      crops: [
        { name: "Cabbage", demandedPrice: 16, requestQty: 950 },
        { name: "Beans", demandedPrice: 50, requestQty: 800 },
      ],
    });
    await acceptMembership({
      ...
      membership: req.membership,
      ts: at(47, 16, 20),
      accepted: [{ deal: req.deals[0].deal, agreedPrice: 18, requestQty: 950 }],
      rejected: [
        {
          deal: req.deals[1].deal,
          reason:
            "Bean demand is already met for this cycle — try again next season.",
        },
      ],
      zone,
      route: "Kalimath → Ukhimath → Rishikesh",
    ...
    await grow({
      ...
      deal: req.deals[0].deal,
      stage: "GROWING",
      message: "Heads firming well.",
      ts: at(30, 9, 0),
      imgSeed: "uk-cabbage-grow-2",
    });
    await grow({
      farmer: F,
      deal: req.deals[0].deal,
      stage: "READY",
      message: "950 kg cabbage ready.",
      ts: at(6, 14, 0),
      imgSeed: "uk-cabbage-ready-2",
    });
    ...
    await pickup({
      ...
      pickupDate: at(1, 9, 0),
      time: "09:00",
      notes: "Ukhimath cabbage pickup — payment pending.",
      startAt: at(1, 8, 30),
      completeAt: at(1, 15, 10),
      items: [
        {
          farmer: F,
          deal: req.deals[0].deal,
  ...
  // ── Story N: TEST ACCOUNTS — Test Farmer Group x Collective Test Hub ───────
  // farmers@gmail.com + collective@gmail.com demoing the full loop:
  // request → partial accept → growth → pickup (one crop paid, one pending) →
  // upcoming pickup → review → announcement.
  {
    const F = testFarmer,
      C = testHub;
    const zone = C.zones[0],
    ...
    const req = await requestMembership({
      ...
      collective: C,
      ts: at(12, 10, 0),
      note: "Test partnership — potato, tomato and cabbage from the demo group.",
      crops: [
        { name: "Potato", demandedPrice: 20, requestQty: 2000 },
        { name: "Tomato", demandedPrice: 24, requestQty: 1500 },
        { name: "Cabbage", demandedPrice: 16, requestQty: 1200 },
      ],
    });
    await acceptMembership({
      farmer: F,
      ...
      ts: at(10, 16, 30),
      accepted: [
        { deal: req.deals[0].deal, agreedPrice: 22, requestQty: 2000 },
        { deal: req.deals[1].deal, agreedPrice: 27, requestQty: 1500 },
      ],
      rejected: [
        {
          deal: req.deals[2].deal,
          reason: "Cabbage volumes are already contracted this season.",
        },
      ],
      zone,
      route: "Rudraprayag → Rishikesh (test route)",
      distance: 175,
      estTime: 220,
    });
    const dPotato = req.deals[0].deal,
    ...
    await grow({
      ...
      deal: dPotato,
      stage: "GROWING",
      message: "Tubers developing well.",
      ts: at(9, 9, 15),
      imgSeed: "test-potato-grow",
    });
    await grow({
      farmer: F,
      deal: dTomato,
      stage: "GROWING",
      message: "Fruit set across all vines.",
      ts: at(8, 10, 0),
      imgSeed: "test-tomato-grow",
    });
    await grow({
      farmer: F,
      deal: dPotato,
      stage: "READY",
      message: "2000 kg ready for pickup.",
      ts: at(5, 15, 0),
      imgSeed: "test-potato-ready",
    });
    await grow({
      farmer: F,
      deal: dTomato,
      ...
      message: "1500 kg ready.",
      ts: at(4, 15, 40),
      imgSeed: "test-tomato-ready",
    });
    await askStatus({ collective: C, deal: dPotato, ts: at(3, 9, 45) });
    await grow({
      farmer: F,
      deal: dPotato,
      stage: "READY",
      message: "Confirmed — crates packed.",
      ts: at(3, 14, 0),
      imgSeed: "test-potato-confirm",
    });
    ...
    await pickup({
      ...
      driver,
      createdAt: at(3, 12, 0),
      pickupDate: at(1, 9, 30),
      time: "09:30",
      notes: "First test-route pickup.",
      startAt: at(1, 8, 45),
      completeAt: at(1, 14, 30),
      items: [
        {
          farmer: F,
          deal: dPotato,
          ...
          collectQty: 1900,
          membership: req.membership,
          remark: "10 crates short at weigh-in",
        },
        {
          farmer: F,
          deal: dTomato,
      ...
      pays: [
        {
          ...
          method: "UPI",
          utr: "UTR-TEST-001",
          proofSeed: "test-pay-1",
          remarks: "Test pickup settlement",
          ts: at(0, 11, 0),
        },
      ],
    });

    // upcoming pickup (+7d) for the remaining potato batch
    const code = await generateId("schedule");
    const schedule = await saveAt(
      Schedule,
      {
        ...
        time: "09:00",
        status: "SCHEDULED",
        totalAmount: round2(1500 * 22),
        totalQuantity: 1500,
        paidAmount: 0,
        farmerCount: 1,
        itemCount: 1,
        notes: "Test potato batch 2.",
      },
      at(0, 12, 0),
    );
    await saveAt(
    ...
    await touchAt(
      ...
      {
        "schedule.activeSchedule": schedule._id,
        "schedule.collectedQuantity": 1500,
      },
      at(0, 12, 0),
    );
    await touchAt(Driver, driver._id, { status: "ASSIGNED" }, at(0, 12, 0));
    await notify({
      recipient: F.user._id,
      recipientRole: "FARMER_GROUP",
      type: "PICKUP",
      title: `Pickup Scheduled · ${code}`,
      body: `Potato (1500 kg) will be collected on ${at(-7, 9, 0).toLocaleDateString("en-IN")} at 09:00 from Test Zone. Estimated value ₹${fmt(round2(1500 * 22))}.`,
      data: {
        scheduleId: schedule._id,
        scheduleCode: code,
        pickupDate: at(-7, 9, 0),
        amount: round2(1500 * 22),
      },
      sender: C.user._id,
      ts: at(0, 12, 30),
    });
    ...
    await reviewCollective({
      ...
      rating: 4,
      comment:
        "Smooth onboarding and quick weigh-ins. Looking forward to the next cycle.",
      ts: at(0, 13, 0),
    });
  }
  ...
  const announcement = async ({
    ...
    const members = WORLD.memberships.filter(
      (m) => m.collective.toString() === collective.profile._id.toString(),
    );
    for (const m of members) {
      await notify({
        ...
        title,
        body,
        data: { announcementId: a._id, newPrice: newPrice ?? null },
        sender: collective.user._id,
        ts,
      });
    }
  ...
  await announcement({
    collective: testHub,
    ts: at(2, 10, 0),
    title: "Test route pickup confirmed",
    body: "Pickup for the test route is confirmed for this week. Please keep crates graded and packed by 8:30 AM.",
    targetCrops: ["Potato"],
    readBy: [testFarmer],
  });
...
const main = async () => {
  ...
  const reset = process.argv.includes("--reset") || force;

  await dbConnect();
  await seedCounters();

  if (reset) {
    // safety: never wipe real (non-bot) registrations unless --force is given
    const real = await User.findOne({
      username: { $nin: BOT_EMAILS },
      role: { $in: ["FARMER_GROUP", "COLLECTIVE"] },
    });
    if (real && !force) {
      console.error(
        `🚫 Refusing to reset: real account found (${real.username}).`,
      );
      console.error(
        "   The world wipe deletes ALL users. Re-run with `--force` to wipe anyway.",
      );
  ...
  try {
    ...
    console.log("🔑 All bot accounts use password: password");
    console.log(
      "   Test logins — farmer: farmers@gmail.com | collective: collective@gmail.com",
    );
  } catch (err) {
