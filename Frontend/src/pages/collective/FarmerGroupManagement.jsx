...
const FGRow = ({ farmer, isDark, isSelected, onClick }) => {
  ...
  const activeDeals = deals.filter((d) => d.status === "APPROVED" || d.status === "REQUESTED");
    ...
    >
      ...
      <div className="relative w-12 h-12 rounded-2xl overflow-hidden shrink-0 bg-gradient-to-br from-emerald-500 via-teal-500 to-emerald-700 shadow-md">
        {farmer.profile || farmer.profilePhoto ? (
          <img src={farmer.profile || farmer.profilePhoto} alt="" className="w-full h-full object-cover" onError={(e) => { e.currentTarget.style.display = "none"; }} />
        ) : (
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1 text-xs">
          ...
          <span className={`truncate flex items-center gap-1 ${isDark ? "text-slate-400" : "text-slate-500"}`}>
            <Icon icon="ph:map-pin-fill" className="w-3.5 h-3.5 text-teal-500 shrink-0" />
...
// ── Detail Panel ──────────────────────────────────────────────────────────────
const DetailPanel = ({ farmer, isDark, onGetStatus, onTerminateDeal, onClose }) => {
  ...
  const PANEL_TABS = [
    ...
    { key: "crops", label: `Dealing Crops (${deals.length})`, icon: "ph:plant-bold" },
  ];
  ...
  return (
    <div className={`rounded-2xl border overflow-hidden ${isDark ? "bg-slate-900 border-slate-800 shadow-2xl shadow-black/40" : "bg-white border-slate-200 shadow-2xl shadow-slate-200/50"}`}>
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-emerald-600 via-teal-700 to-slate-900 p-5 pb-5">
        ...
        <div className="relative z-10 flex items-end gap-4">
          ...
          <div className="flex-1 min-w-0 pb-1">
            <h2 className="text-xl font-bold text-white leading-tight truncate">{farmer.name || farmer.groupName || "Farmer Group"}</h2>
            <p className="text-xs text-white/80 mt-0.5 truncate">
              Lead Farmer: <strong className="text-white">{farmer.leadFarmer || "N/A"}</strong> · {farmer.farmerCount || farmer.numberOfFarmers || 1} Farmers
            </p>
      ...
      {/* Subsections Tab Bar */}
      <div className={`flex border-b ${isDark ? "border-slate-800 bg-slate-900/50" : "border-slate-200 bg-slate-50/50"}`}>
        {PANEL_TABS.map((t) => (
          <button
            key={t.key}
      ...
      <div className="p-4">
        <AnimatePresence mode="wait">
            ...
            <motion.div key="overview" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
              ...
              <div className={`rounded-xl p-3.5 border ${isDark ? "bg-slate-800/50 border-slate-700/50" : "bg-slate-50 border-slate-200"}`}>
                ...
                <div className="space-y-2.5">
                  {farmer.phone && (
                    <a href={`tel:${farmer.phone}`} className="flex items-center gap-2.5 group/link">
                      ...
                      <div>
                        <p className={`text-xs font-bold group-hover/link:text-emerald-500 transition-colors ${isDark ? "text-white" : "text-slate-900"}`}>
                          {farmer.phone}
                        </p>
                  ...
                  {farmer.email && (
                    <a href={`mailto:${farmer.email}`} className="flex items-center gap-2.5">
                      ...
                      <p className={`text-xs font-medium ${isDark ? "text-slate-300" : "text-slate-600"}`}>{farmer.email}</p>
                    </a>
                  )}
                  <div className="flex items-center gap-2.5">
                    ...
                    <p className={`text-xs font-medium ${isDark ? "text-slate-300" : "text-slate-600"}`}>{formatAddress(farmer.address)}</p>
                  </div>
                  {farmer.membership?.memberSince && (
                    <div className="flex items-center gap-2.5 pt-1">
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${isDark ? "bg-amber-500/15" : "bg-amber-50"}`}>
                        <Icon icon="ph:calendar-bold" className="w-4 h-4 text-amber-500" />
                      </div>
                      ...
                      {grownCropsList.map((cName) => (
                        <span
                          ...
                          className={`flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full border ${
                            isDark ? "bg-slate-900/80 border-slate-700 text-emerald-400" : "bg-white border-slate-200 text-emerald-700 shadow-sm"
                          }`}
                        >
                          <Icon icon="ph:leaf-fill" className="w-3 h-3 text-emerald-500" />
                          {cName}
                        </span>
                      ))}
                    </div>
          ...
          {panelTab === "crops" && (
            <motion.div key="crops" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-3">
              {deals.length === 0 ? (
                <div className={`rounded-xl p-6 border text-center ${isDark ? "bg-slate-800/40 border-slate-700/50" : "bg-slate-100 border-slate-200"}`}>
                  <Icon icon="ph:plant-fill" className={`w-8 h-8 mx-auto mb-2 ${isDark ? "text-slate-500" : "text-slate-400"}`} />
                  <p className={`text-sm font-semibold ${isDark ? "text-slate-400" : "text-slate-500"}`}>No crop deals associated</p>
                  <p className={`text-xs mt-1 ${isDark ? "text-slate-600" : "text-slate-400"}`}>Partnered crop deals will appear here.</p>
                </div>
              ) : (
                deals.map((deal) => {
                  ...
                  const isQueryPending = deal.queryPending || deal.growth?.queryStatus === "OPEN";

                  const statusStyle = {
                    APPROVED: "bg-emerald-500/15 text-emerald-400 border-emerald-500/30",
                    REQUESTED: "bg-amber-500/15 text-amber-400 border-amber-500/30",
                    REJECTED: "bg-red-500/15 text-red-400 border-red-500/30",
                    F_TERMINATE: "bg-red-500/15 text-red-400 border-red-500/30",
                    C_TERMINATE: "bg-red-500/15 text-red-400 border-red-500/30",
                  ...
                  const daysRemaining = Math.max(1, Math.ceil(10 - daysSinceUpdate));
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-center gap-2.5">
                          <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${isDark ? "bg-slate-800 text-emerald-400" : "bg-emerald-100 text-emerald-600"}`}>
                            <Icon icon="ph:plant-fill" className="w-5 h-5" />
                          </div>
                          <div>
                            <h4 className="font-bold text-sm leading-snug">{cropName}</h4>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div>
                          ...
                          <span className={`font-extrabold ${isDark ? "text-emerald-400" : "text-emerald-600"}`}>
                            {deal.agreedPrice > 0 ? `₹${deal.agreedPrice}/kg` : deal.demandedPrice > 0 ? `₹${deal.demandedPrice}/kg` : "—"}
                          </span>
                        </div>
                        <div>
                          <span className={`text-[10px] block font-semibold ${isDark ? "text-slate-400" : "text-slate-500"}`}>Growth Stage</span>
                          ...
                          {isApproved && (
                            isQueryPending ? (
                              <span className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-amber-500/15 text-amber-400 border border-amber-500/30 flex items-center gap-1">
                                <Icon icon="ph:hourglass-medium-bold" className="w-3.5 h-3.5 animate-spin" />
                                Query Pending
                              </span>
                            ) : !canRequestStatus ? (
                              <button
...
const FarmerGroupManagement = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const mData = memberRes.data?.memberData || { requests: [], approved: [], rejected: [], terminated: [], cancelled: [] };
      setMemberData(mData);
  ...
  const membersList = Array.isArray(memberData.approved) ? memberData.approved : [];
  const requestsList = Array.isArray(memberData.requests) ? memberData.requests : [];
  ...
  const filteredMembers = useMemo(() => {
    ...
    return membersList.filter((f) => {
      ...
      return nameMatch || leadMatch || emailMatch || phoneMatch || addrMatch || cropMatch;
    });
  }, [membersList, search]);
  ...
  useEffect(() => {
    if (tab === "members" && view === "list") {
      if (filteredMembers.length > 0) {
        if (!selectedFarmer || !filteredMembers.some((f) => f._id === selectedFarmer._id)) {
  ...
  useEffect(() => {
    if (selectedFarmer) {
      const updated = membersList.find((f) => f._id === selectedFarmer._id) || requestsList.find((f) => f._id === selectedFarmer._id);
  ...
  const handleReviewSubmit = async () => {
    ...
    for (const deal of selectedFarmer.deals || []) {
      const dec = cropDecisions[deal._id] || { action: "ACCEPT", price: "", reason: "" };
      const cropName = deal.crop?.crop?.name || deal.crop?.name || "Crop";

      if (dec.action === "ACCEPT") {
        const price = Number(dec.price);
        if (!price || price <= 0) {
          toast.error(`Please enter a valid agreed price for accepted crop: ${cropName}`);
          return;
        }
    ...
    try {
      ...
      toast.error(err.response?.data?.message || "Failed to process membership request");
    } finally {
      setApproving(false);
    }
  };
  ...
  const handleGetStatus = async (dealId) => {
    ...
    try {
      ...
      toast.success(res.data?.message || "Status request sent to farmer group!");
      fetchData();
    } catch (err) {
      toast.error(err.response?.data?.message || "Failed to request crop status");
    }
  };
  ...
  const handleTerminateDeal = async (dealId, cropName) => {
    if (!dealId) return;
    if (!window.confirm(`Are you sure you want to terminate partnership for ${cropName || "this crop"}?`)) return;
    try {
      const res = await collectiveMemberAPI.terminate({ dealId, reason: "Terminated by collective" });
        ...
        {view === "list" ? (
          <motion.div key="list" variants={pageVariants} initial="initial" animate="animate" exit="exit">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              <div>
                ...
                <p className={`text-xs sm:text-sm mt-1 ${isDark ? "text-slate-400" : "text-slate-500"}`}>
                  Browse, review, and manage associated farmer groups and crop partnerships
                </p>
              </div>
              ...
              <div className="flex items-center gap-2.5">
                {[
                  { label: "Active Members", value: membersList.length, icon: "ph:users-three-fill", color: "text-emerald-500" },
                  { label: "Pending Requests", value: requestsList.length, icon: "ph:clock-fill", color: "text-amber-500" },
                  { label: "Total Zones", value: zones.length, icon: "ph:map-trifold-fill", color: "text-blue-500" },
                ].map((s) => (
            ...
            <div className="flex items-center justify-between gap-4 mb-6 flex-wrap">
              <div className={`flex gap-1.5 p-1.5 rounded-2xl border backdrop-blur-md ${isDark ? "bg-slate-900/60 border-slate-800" : "bg-white border-slate-200 shadow-sm"}`}>
                {[
                  { id: "members", label: "Active Members", count: membersList.length, icon: "ph:users-three-bold" },
                  { id: "requests", label: "Pending Requests", count: requestsList.length, icon: "ph:clock-bold", badge: requestsList.length > 0 },
                ].map((t) => (
                  <button
                    key={t.id}
                    onClick={() => setTab(t.id)}
                  ...
                  >
                    <Icon icon={t.icon} className="w-4 h-4" />
                    {t.label}
                    <span className={`text-xs px-2 py-0.5 rounded-full ${tab === t.id ? "bg-white/25 text-white" : isDark ? "bg-slate-800 text-slate-300" : "bg-slate-100 text-slate-600"}`}>
                      {t.count}
                    </span>
                    {t.badge && tab !== t.id && (
            ...
            {loading ? (
              ...
              filteredMembers.length === 0 ? (
                <EmptyState
                  ...
                  description={search ? "No farmer group matched your search query." : "You have no active farmer groups in your network yet."}
                />
              ) : (
                    ...
                    {requestsList.map((farmer) => (
                      <div
                        ...
                        className={`flex flex-col md:flex-row items-start md:items-center justify-between gap-5 p-5 rounded-2xl border backdrop-blur-xl ${
                          isDark ? "bg-slate-900/60 border-slate-800/80 shadow-xl shadow-black/20" : "bg-white border-slate-200 shadow-md"
                        }`}
                      >
                        <div className="flex items-center gap-4 min-w-0">
                          <div className={`w-14 h-14 rounded-2xl overflow-hidden shrink-0 flex items-center justify-center font-bold text-lg ${
                            isDark ? "bg-slate-800 text-emerald-400" : "bg-emerald-50 text-emerald-600"
                          }`}>
                            {farmer.profile || farmer.profilePhoto ? (
                              <img src={farmer.profile || farmer.profilePhoto} alt="" className="w-full h-full object-cover" />
                            ) : (
                              (farmer.name || farmer.groupName || "FG").substring(0, 2).toUpperCase()
                            )}
                          </div>
                          <div>
                            ...
                            <p className={`text-xs mt-0.5 ${isDark ? "text-slate-400" : "text-slate-500"}`}>
                              Leader: <strong className={isDark ? "text-slate-200" : "text-slate-700"}>{farmer.leadFarmer || "N/A"}</strong> · {farmer.farmerCount || 1} farmers · {shortAddr(farmer.address)}
                            </p>
                            <div className="flex flex-wrap gap-1.5 mt-2">
                              {(farmer.deals || []).map((d) => {
                                const cropName = d.crop?.crop?.name || d.crop?.name || "Crop";
                                return (
                                  ...
                                  >
                                    {cropName} {d.demandedPrice > 0 ? `(Demand: ₹${d.demandedPrice}/kg)` : ""}
                                  </span>
                                );
                              })}
                            </div>
          ...
          <motion.div key="approve_form" variants={formVariants} initial="initial" animate="animate" exit="exit" className="max-w-2xl mx-auto">
            <button
            ...
            <div className={`rounded-2xl border p-5 sm:p-6 backdrop-blur-xl shadow-xl ${
              ...
              <div className="flex items-center gap-4 mb-4 pb-4 border-b border-slate-200 dark:border-slate-800">
                ...
                <div>
                  <h2 className="text-xl font-bold">Review Membership Request</h2>
                  <p className={`text-xs ${isDark ? "text-slate-400" : "text-slate-500"}`}>
                    Review crops requested by {selectedFarmer?.name || selectedFarmer?.groupName}. Accept or reject crops individually, set prices, and assign a zone.
                  </p>
                </div>
              </div>
