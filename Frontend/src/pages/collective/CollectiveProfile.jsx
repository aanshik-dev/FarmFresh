...
const CROPS_LIST = ["Rajma", "Potato", "Ginger", "Garlic", "Wheat", "Barley", "Peas", "Mustard", "Soyabean", "Buckwheat"];
...
const CollectiveProfile = () => {
  ...
  // Editable fields
  const [collectiveName, setCollectiveName] = useState(user?.collectiveName || profile?.name || "");
  const [phone, setPhone] = useState(user?.phone || profile?.phone || "");
  const [workers, setWorkers] = useState(user?.workers || profile?.workers || "");
  const [description, setDescription] = useState(user?.description || "Premier organic produce collective in the Kedarnath corridor, operating since 2018.");
  const [selectedCrops, setSelectedCrops] = useState(user?.crops || profile?.crops?.map(c => c.name) || []);

  // Address fields
  const [village, setVillage] = useState(user?.village || "");
  const [district, setDistrict] = useState(user?.district || "Rudraprayag District");
  ...
  const handleSave = async () => {
    ...
    const address = [village, district, state, pincode].filter(Boolean).join(", ");
    updateUser({
      collectiveName, phone, workers: parseInt(workers) || workers,
      description, crops: selectedCrops, address, village, district, state, pincode,
      coordinates: lat && lng ? { lat: parseFloat(lat), lng: parseFloat(lng) } : user?.coordinates,
  ...
  const fullAddress = [user?.village, user?.district, user?.state, user?.pincode].filter(Boolean).join(", ") || profile?.address;
      ...
      <div className="max-w-3xl mx-auto space-y-6">
          ...
          <div className="px-6 pb-6">
            <div className="flex items-end justify-between -mt-10 mb-4 flex-wrap gap-3">
              ...
              <div className="flex gap-2">
                {editing && (
                  <button onClick={handleCancel} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium cursor-pointer transition-all border ${isDark ? "border-slate-700 text-slate-400 hover:bg-slate-800" : "border-slate-200 text-slate-500 hover:bg-slate-50"}`}>
                    Cancel
                  </button>
                )}
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium cursor-pointer transition-all ${
                    editing ? "bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-lg" : isDark ? "border border-slate-700 text-slate-300 hover:bg-slate-800" : "border border-slate-200 text-slate-600 hover:bg-slate-50"
                  }`}
                >
            ...
            <p className={`text-xs mb-4 ${isDark ? "text-slate-500" : "text-slate-400"}`}>
              <Icon icon="ph:lock-fill" className="w-3 h-3 inline mr-1" />Email: <span className={isDark ? "text-slate-400" : "text-slate-500"}>{user?.email}</span> (cannot be changed)
            </p>
            ...
            {editing ? (
              <div className="space-y-4">
                <div className="grid sm:grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <label className={`block text-sm font-medium ${isDark ? "text-slate-300" : "text-slate-700"}`}>Collective Name</label>
                    <input className={iClass} value={collectiveName} onChange={e => setCollectiveName(e.target.value)} placeholder="Collective name" />
                  </div>
                  <div className="space-y-1.5">
                    <label className={`block text-sm font-medium ${isDark ? "text-slate-300" : "text-slate-700"}`}>Phone</label>
                    <input className={iClass} value={phone} onChange={e => setPhone(e.target.value)} placeholder="+91..." />
                  </div>
                  <div className="space-y-1.5">
                    <label className={`block text-sm font-medium ${isDark ? "text-slate-300" : "text-slate-700"}`}>Total Workers</label>
                    <input className={iClass} type="number" value={workers} onChange={e => setWorkers(e.target.value)} placeholder="25" />
                  </div>
                </div>
                <div className="space-y-1.5">
                  <label className={`block text-sm font-medium ${isDark ? "text-slate-300" : "text-slate-700"}`}>Description</label>
                  <textarea value={description} onChange={e => setDescription(e.target.value)} rows={3} placeholder="About your collective…" className={`${iClass} resize-none`} />
                </div>

                {/* Address */}
                <div className={`rounded-xl border p-4 space-y-3 ${isDark ? "border-slate-700 bg-slate-800/30" : "border-slate-200 bg-slate-50"}`}>
                  <p className={`text-sm font-semibold ${isDark ? "text-slate-300" : "text-slate-700"}`}>Address & Location</p>
                  <div className="grid sm:grid-cols-2 gap-3">
                    {[
                      { label: "Village / Town", val: village, set: setVillage, ph: "e.g. Ukhimath" },
                      { label: "District", val: district, set: setDistrict, ph: "e.g. Rudraprayag" },
                      { label: "State", val: state, set: setState, ph: "e.g. Uttarakhand" },
                      { label: "Pincode", val: pincode, set: setPincode, ph: "246444" },
                      { label: "Latitude", val: String(lat), set: setLat, ph: "30.7333" },
                      { label: "Longitude", val: String(lng), set: setLng, ph: "79.0667" },
                    ].map(f => (
                      <div key={f.label} className="space-y-1.5">
                        <label className={`block text-xs font-medium ${isDark ? "text-slate-400" : "text-slate-600"}`}>{f.label}</label>
                        <input className={iClass} value={f.val} onChange={e => f.set(e.target.value)} placeholder={f.ph} />
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <p className={`text-sm font-medium ${isDark ? "text-slate-300" : "text-slate-700"}`}>Crops Handled</p>
                  <div className="flex flex-wrap gap-2">
                    {CROPS_LIST.map(crop => (
                      <button key={crop} type="button" onClick={() => toggleCrop(crop)}
                        className={`px-3 py-1.5 rounded-xl text-xs font-medium border transition-all cursor-pointer ${
                          selectedCrops.includes(crop)
                            ? isDark ? "bg-blue-500/20 border-blue-500/50 text-blue-300" : "bg-blue-100 border-blue-300 text-blue-700"
              ...
              <div className="space-y-3">
                <p className={`text-sm leading-relaxed ${isDark ? "text-slate-300" : "text-slate-600"}`}>
                  {user?.description || description || <span className={`italic ${isDark ? "text-slate-500" : "text-slate-400"}`}>No description added yet.</span>}
                </p>
                <div className="flex flex-wrap gap-4 text-sm">
                  {[
                    { icon: "ph:map-pin-fill", val: fullAddress },
                    { icon: "ph:phone-fill", val: user?.phone || profile?.phone },
                    { icon: "ph:envelope-fill", val: user?.email || profile?.email },
                  ].filter(i => i.val).map(item => (
                    <span key={item.val} className={`flex items-center gap-1.5 ${isDark ? "text-slate-400" : "text-slate-500"}`}>
                      <Icon icon={item.icon} className="w-4 h-4 shrink-0" />{item.val}
                    </span>
                  ))}
                </div>
                {user?.coordinates?.lat && (
                  <p className={`text-xs ${isDark ? "text-slate-500" : "text-slate-400"}`}>
                    <Icon icon="ph:map-trifold-fill" className="w-3.5 h-3.5 inline mr-1" />
        ...
        {!editing && (
          <div className={`rounded-2xl border p-5 ${isDark ? "bg-slate-900/60 border-slate-800" : "bg-white border-slate-200"}`}>
            <h2 className={`font-semibold mb-4 ${isDark ? "text-white" : "text-slate-900"}`}>Crops Handled</h2>
            <div className="flex flex-wrap gap-2">
              {(user?.crops || profile?.crops?.map(c => c.name) || []).map(crop => (
                <span key={crop} className={`px-3 py-1.5 rounded-xl text-xs font-medium border ${isDark ? "bg-blue-500/10 border-blue-500/20 text-blue-400" : "bg-blue-50 border-blue-200 text-blue-700"}`}>{crop}</span>
              ))}
            </div>
          </div>
        )}
