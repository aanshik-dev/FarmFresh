...
const PickupScheduler = () => {
  ...
  const handleCreate = async () => {
    ...
    toast.success("Pickup scheduled! Farmer group has been notified.", {
      title: "Scheduled",
    });
    setShowModal(false);
    setForm({
    ...
    >
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          >
            {active.length} active · {schedules.length} total
          </p>
        </div>
        <button
      ...
      >
        {[
          ...
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all cursor-pointer ${tab === t.id ? "bg-linear-to-r from-emerald-500 to-emerald-600 text-white shadow" : isDark ? "text-slate-400 hover:text-slate-200" : "text-slate-500 hover:text-slate-700"}`}
          >
            {t.label}
          </button>
        ))}
      ...
      {displayed.length === 0 ? (
        <EmptyState
          ...
          title="No schedules"
          description="Create a pickup schedule to get started."
          action={
            <button
              onClick={() => setShowModal(true)}
              className="px-4 py-2 rounded-xl bg-emerald-500 text-white text-sm cursor-pointer"
            >
              Create Schedule
            </button>
          }
        ...
        <div className="space-y-4">
          {displayed.map((s, i) => (
            ...
            >
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    >
                      {s.farmerGroup.split(" ").slice(0, 3).join(" ")}
                    </h3>
                ...
                <div
                  className={`shrink-0 text-right px-3 py-2 rounded-xl ${isDark ? "bg-slate-800" : "bg-slate-50"}`}
                >
                  <p
              ...
              <div
                className={`flex items-center gap-2 mt-3 text-sm ${isDark ? "text-slate-400" : "text-slate-500"}`}
              >
              ...
              {s.status === "scheduled" && (
                <div className="flex gap-2 mt-3">
                  ...
                  <button
                    ...
                    className={`flex-1 py-2 rounded-xl text-xs font-medium bg-red-500/15 text-red-400 border border-red-500/25 cursor-pointer hover:bg-red-500/25`}
                  >
                    Cancel
                  </button>
                </div>
              ...
              {s.status === "in_progress" && (
                <button
                  ...
                  className="mt-3 w-full py-2 rounded-xl text-xs font-medium bg-emerald-500/15 text-emerald-400 border border-emerald-500/25 cursor-pointer hover:bg-emerald-500/25"
                >
                  Mark Completed
                </button>
              )}
        ...
        {showModal && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className={`relative w-full max-w-lg rounded-2xl border p-6 shadow-2xl z-10 overflow-y-auto max-h-[90vh] ${isDark ? "bg-slate-900 border-slate-700" : "bg-white border-slate-200"}`}
            >
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select
                    value={form.farmerGroupId}
                    onChange={(e) =>
                      setForm((p) => ({
                        ...p,
                        farmerGroupId: e.target.value,
                        crop: "",
                      }))
                    }
                ...
                {selectedGroup && (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedGroup.crops.map((c) => (
                        <button
                          key={c}
                          type="button"
                          onClick={() => setForm((p) => ({ ...p, crop: c }))}
                          className={`px-3 py-1.5 rounded-full text-sm border cursor-pointer ${form.crop === c ? "bg-emerald-500 border-emerald-500 text-white" : isDark ? "border-slate-700 text-slate-400" : "border-slate-200 text-slate-500"}`}
                        >
                          {c}
                        </button>
                      ))}
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <select
                      value={form.driverId}
                      onChange={(e) =>
                        setForm((p) => ({ ...p, driverId: e.target.value }))
                      }
                      className={`w-full rounded-xl border px-3 py-2.5 text-sm outline-none ${isDark ? "bg-slate-800 border-slate-700 text-white" : "bg-white border-slate-200 text-slate-900"}`}
                    >
              ...
              <div className="flex gap-2 mt-5">
                ...
                <button
                  ...
                  disabled={creating}
                  className={`flex-1 py-2.5 rounded-xl text-sm font-semibold bg-linear-to-r from-emerald-500 to-emerald-600 text-white cursor-pointer flex items-center justify-center gap-2 ${creating ? "opacity-70" : ""}`}
                >
                  {creating && (
