        ...
        <div className="max-w-3xl space-y-4">
          {announcements.map((a, i) => (
            <motion.div
              ...
              className={`rounded-2xl border p-5 ${a.pinned ? (isDark ? "border-amber-500/30 bg-amber-500/5" : "border-amber-200 bg-amber-50") : isDark ? "bg-slate-900/60 border-slate-800" : "bg-white border-slate-200"}`}
            >
              <div className="flex items-start justify-between gap-3 mb-3">
                ...
                <div className="flex gap-1 shrink-0">
                  <button
                    ...
                    className={`p-1.5 rounded-lg cursor-pointer ${isDark ? "hover:bg-slate-800 text-slate-400" : "hover:bg-slate-100 text-slate-500"} ${a.pinned ? "text-amber-400!" : ""}`}
                  >
                    <Icon
                      icon={
                        a.pinned ? "ph:push-pin-slash-fill" : "ph:push-pin-fill"
                      }
                      className="w-4 h-4"
                    />
                  </button>
              ...
              <p
                className={`text-sm leading-relaxed mb-3 ${isDark ? "text-slate-300" : "text-slate-600"}`}
              >
                {a.content}
              </p>
              <div className="flex items-center justify-between flex-wrap gap-2 text-xs">
                <div className="flex items-center gap-3">
                  ...
                  >
                    <Icon icon="ph:users-three-fill" className="w-3 h-3" />
                    {a.target}
                  </span>
                  <span
                    className={`flex items-center gap-1 ${isDark ? "text-slate-500" : "text-slate-400"}`}
                  >
                    <Icon icon="ph:eye-fill" className="w-3 h-3" />
              ...
              {/* Read progress bar */}
              <div
                className={`mt-3 h-1.5 rounded-full ${isDark ? "bg-slate-700" : "bg-slate-200"}`}
              >
                <div
                  className="h-full rounded-full bg-emerald-500 transition-all"
                  style={{
                    width: `${(a.readCount / a.totalRecipients) * 100}%`,
                  }}
                />
              </div>
            </motion.div>
        ...
        {showModal && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className={`relative w-full max-w-lg rounded-2xl border p-6 shadow-2xl z-10 ${isDark ? "bg-slate-900 border-slate-700" : "bg-white border-slate-200"}`}
            >
              <h3
                className={`font-bold text-xl mb-5 ${isDark ? "text-white" : "text-slate-900"}`}
              >
                New Announcement
              </h3>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {TARGETS.map((t) => (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-full text-xs border cursor-pointer ${form.target === t ? "bg-emerald-500 border-emerald-500 text-white" : isDark ? "border-slate-700 text-slate-400 hover:border-slate-500" : "border-slate-200 text-slate-500 hover:border-slate-300"}`}
                      >
                        {t}
                      </button>
                    ))}
