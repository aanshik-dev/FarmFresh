...
const StatCard = ({ label, value, icon, sub, color = "emerald" }) => {
  ...
  return (
    ...
    >
      <div
        className={`w-11 h-11 rounded-xl flex items-center justify-center bg-linear-to-br ${c} shadow-lg mb-4`}
      >
      ...
      {sub && (
        <p
          className={`text-xs mt-1 ${isDark ? "text-emerald-400" : "text-emerald-600"}`}
        >
          {sub}
        </p>
      )}
...
const CollectiveDashboard = () => {
  ...
  const upcoming = pickupSchedules.filter(
    (s) => s.status === "scheduled",
  ).length;
  const pending = membershipRequests.filter(
    (m) => m.status === "pending",
  ).length;
    ...
    >
      ...
      <div className="flex items-start justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          >
            {user?.collectiveName || "Collective Overview"} ·{" "}
        ...
        <div className="flex gap-2 flex-wrap">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium border cursor-pointer transition-all ${isDark ? "border-slate-700 text-slate-300 hover:bg-slate-800" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold bg-linear-to-r from-emerald-500 to-emerald-600 text-white cursor-pointer shadow-lg shadow-emerald-500/20"
          >
      ...
      {(pending > 0 || unread > 0) && (
        <div className="flex flex-wrap gap-3 mb-6">
          {pending > 0 && (
            <div
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border cursor-pointer text-sm ${isDark ? "bg-amber-500/10 border-amber-500/25 text-amber-400" : "bg-amber-50 border-amber-200 text-amber-700"}`}
              onClick={() => navigate("/dashboard/collective/farmers")}
            >
              <Icon icon="ph:user-plus-fill" className="w-4 h-4" />
              <span>
                {pending} membership request{pending > 1 ? "s" : ""} pending
                review
              </span>
          ...
          {unread > 0 && (
            <div
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border cursor-pointer text-sm ${isDark ? "bg-blue-500/10 border-blue-500/25 text-blue-400" : "bg-blue-50 border-blue-200 text-blue-700"}`}
              onClick={() => navigate("/dashboard/collective/notifications")}
            >
          ...
          <table className="w-full text-sm">
            ...
            >
              {pickupSchedules.slice(0, 4).map((s) => (
                ...
                >
                  <td
                    className={`py-3 pr-4 font-medium ${isDark ? "text-slate-200" : "text-slate-800"}`}
                  >
                    {s.farmerGroup.split(" ").slice(0, 2).join(" ")}
                  </td>
                  <td
