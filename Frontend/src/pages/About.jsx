...
const About = () => {
  ...
  return (
    <div
      className={`w-full min-h-screen overflow-hidden transition-colors duration-500 ${isDark ? "bg-slate-950" : "bg-slate-50"}`}
    >
      {/* Hero Section */}
      <section className="relative pt-32 pb-20 px-6 sm:px-10 lg:px-20">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          >
            ...
            />
            <span
              className={`text-sm font-semibold tracking-wide uppercase ${isDark ? "text-slate-300" : "text-slate-600"}`}
            >
          ...
          >
            Coordinating Himalayan harvests{" "}
          ...
          >
            Mandakini Organic began as a handful of farmer groups sharing a
            single harvest calendar by phone. Today the collective spans four
            altitude zones across the Kedarnath Valley, coordinating organic
            cultivation, collection logistics, and sustainable livelihoods for
      ...
      <section className="px-6 sm:px-10 lg:px-20 py-20 relative z-10">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <h2
              className={`font-display text-3xl sm:text-4xl font-bold mb-4 ${isDark ? "text-white" : "text-slate-900"}`}
            >
              What we stand for
            </h2>
            <p
            ...
            >
              Our core values guide every decision we make for the collective
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
            {values.map((v, i) => (
              ...
              >
                <div
                  className={`shrink-0 p-4 rounded-2xl h-fit transition-transform duration-300 group-hover:scale-110 group-hover:-rotate-3 ${isDark ? "bg-emerald-500/10 text-emerald-400" : "bg-emerald-100 text-emerald-600"}`}
                >
                ...
                <div>
                  ...
                  <p
                    className={`leading-relaxed ${isDark ? "text-slate-400 group-hover:text-slate-300" : "text-slate-600"} transition-colors`}
                  >
                    {v.text}
                  </p>
                </div>
      ...
      {/* Timeline Section */}
      <section
        className={`px-6 sm:px-10 lg:px-20 py-24 ${isDark ? "bg-slate-900/30 border-t border-slate-800/50" : "bg-slate-100/50 border-t border-slate-200"}`}
      >
        <div className="max-w-4xl mx-auto">
          ...
          <div className="relative wrap overflow-hidden p-4 h-full">
            {/* Center line */}
            <div
              className={`absolute left-[20px] md:left-1/2 top-0 bottom-0 w-1 md:-ml-[1px] rounded-full ${isDark ? "bg-slate-800" : "bg-slate-200"}`}
            />
              ...
              {timeline.map((t, i) => {
                ...
                return (
                  ...
                  >
                    {/* Dot */}
                    <div
                      className={`absolute left-[12px] md:left-1/2 w-5 h-5 rounded-full border-4 z-10 md:-translate-x-1/2 shadow-lg ${
                        isDark
                          ? "bg-emerald-400 border-slate-900 shadow-emerald-900/50"
                          : "bg-emerald-500 border-white shadow-emerald-200"
                      }`}
                    />

                    {/* Content Card */}
                    <div
                      className={`w-full md:w-[45%] pl-12 md:pl-0 ${isEven ? "md:mr-auto md:pr-12 md:text-right" : "md:ml-auto md:pl-12"}`}
                    >
                      <div
                      ...
                      >
                        <span
                          className={`inline-block px-3 py-1 rounded-full text-xs font-bold tracking-widest mb-3 ${isDark ? "bg-emerald-500/20 text-emerald-300" : "bg-emerald-100 text-emerald-700"}`}
                        >
                          {t.year}
                        </span>
                        <h3
