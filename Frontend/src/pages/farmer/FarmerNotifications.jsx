...
import { farmerNotifications } from "../../utils/InterfaceData";
import EmptyState from "../../components/common/EmptyState";

const FarmerNotifications = () => {
  const { isDark } = useTheme();
  const [notifs, setNotifs] = useState(farmerNotifications);
  ...
  const TYPE_COLORS = {
    ...
    membership:     "text-emerald-400 bg-emerald-500/10",
    status_request: "text-amber-400 bg-amber-500/10",
    announcement:   "text-violet-400 bg-violet-500/10",
  ...
  return (
    <div className={`min-h-screen p-5 sm:p-7 ${isDark ? "bg-slate-950" : "bg-slate-50"}`}>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        ...
        {unread > 0 && (
          <button onClick={markAllRead} className="text-sm text-emerald-500 hover:text-emerald-400 cursor-pointer font-medium">Mark all as read</button>
        )}
      </div>

      {notifs.length === 0 ? (
        <EmptyState icon="ph:bell-slash-fill" title="No notifications" description="You're all caught up! Notifications from your collectives will appear here." size="lg" />
      ) : (
          ...
          {notifs.map((n, i) => (
            <motion.div
              ...
              className={`relative flex items-start gap-4 p-4 rounded-2xl border cursor-pointer transition-all ${
                !n.read
                  ? isDark ? "bg-slate-900/80 border-emerald-800/30" : "bg-white border-emerald-200"
                  : isDark ? "bg-slate-900/40 border-slate-800/60" : "bg-white/60 border-slate-200"
              }`}
            >
              {!n.read && (
              ...
              <button
                ...
                className={`shrink-0 w-7 h-7 flex items-center justify-center rounded-lg transition-colors ${isDark ? "hover:bg-slate-800 text-slate-600 hover:text-slate-300" : "hover:bg-slate-100 text-slate-300 hover:text-slate-600"}`}
              >
