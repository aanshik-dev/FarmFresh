    ...
    >
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <span
            className={`shrink-0 text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded-full ${crop.status === "ACTIVE" ? "bg-emerald-500/15 text-emerald-500" : "bg-slate-500/15 text-slate-400"}`}
          >
            {crop.status}
          </span>
        </div>
        ...
        {/* Compact stats */}
        <div
          className={`flex items-center gap-3 px-3 py-2 rounded-xl mb-3 text-xs ${isDark ? "bg-slate-950/50" : "bg-slate-50"}`}
        >
          <div className="flex items-center gap-1.5">
            <Icon
              icon="ph:scales-fill"
              className={`w-3.5 h-3.5 shrink-0 ${isDark ? "text-slate-500" : "text-slate-400"}`}
            />
          ...
          <div
            className={`w-px h-3 ${isDark ? "bg-slate-700" : "bg-slate-200"}`}
          />
          <div className="flex items-center gap-1.5">
            <Icon
...
const DetailPanel = ({
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      const res = await farmerDealAPI.getHistory(deal._id);
      setStatusHistory(res.data?.history || res.data?.data || []);
    } catch {
      setStatusHistory([]);
    } finally {
  ...
  useEffect(() => {
    if (panelTab === "history") loadHistory();
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      await farmerDealAPI.updateStatus(deal._id, {
        ...
        message: updateForm.note || undefined,
        imgUrl: updateForm.imgUrl || undefined,
      });
        ...
        <div className="relative z-10 p-5 pb-4">
          <div className="flex items-end gap-4">
            <div className="relative w-20 h-20 rounded-2xl overflow-hidden shrink-0 bg-white/20 ring-2 ring-white/30">
              ...
              <div
                className={`absolute inset-0 flex items-center justify-center text-white ${crop?.crop?.image ? "hidden" : "flex"}`}
              >
                <Icon
                  icon={getCategoryIcon(crop?.crop?.category)}
            ...
            <div className="flex-1 min-w-0 pb-1">
              ...
              <div className="flex flex-wrap gap-1.5 mt-1.5">
                ...
                <span
                  className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${crop?.status === "ACTIVE" ? "bg-emerald-400/40 text-white" : "bg-white/15 text-white/70"}`}
                >
                  {crop?.status}
                </span>
              </div>
        ...
        <AnimatePresence mode="wait">
            ...
            >
              ...
              {isLinked && deal ? (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3">
                        <div className="relative w-12 h-12 rounded-2xl overflow-hidden shrink-0 bg-emerald-500/20 ring-1 ring-emerald-400/30">
                          ...
                          <div
                            className={`absolute inset-0 flex items-center justify-center ${collective.profile ? "hidden" : "flex"}`}
                          >
                            <Icon
                              icon="ph:buildings-fill"
                    ...
                    <div className="grid grid-cols-2 gap-2.5">
                      {[
                        ...
                        { label: "Deal Status", value: deal.status ?? "—" },
                        {
                          label: "Growth Stage",
                          value:
                            STAGE_LABEL[deal.growth?.stage] ??
                            deal.growth?.stage ??
                            "—",
                        },
                        {
                          label: "Approved",
                          value: fmtDate(deal.approvalDate),
                        },
                        {
                          label: "Exp. Qty",
                          value: deal.growth?.expectedQuantity
                            ? `${deal.growth.expectedQuantity} kg`
                            : "—",
                        },
                      ].map(({ label, value }) => (
                        <div key={label}>
                ...
                >
                  <Icon
                    icon="ph:link-break"
                    className={`w-8 h-8 mx-auto mb-2 ${isDark ? "text-slate-500" : "text-slate-400"}`}
                  />
                  <p
                  ...
                  >
                    Browse Collectives to send a membership request.
                  </p>
                </div>
              )}

              <div
                className={`flex gap-2 pt-1 border-t ${isDark ? "border-slate-800" : "border-slate-200"}`}
              >
                <button
                  onClick={() => onEdit(crop)}
          ...
          {panelTab === "update" && (
            ...
            >
              {isQueryOpen ? (
                <div
                  className={`rounded-xl p-3 border flex items-start gap-2 ${isDark ? "bg-amber-950/40 border-amber-500/30" : "bg-amber-50 border-amber-300"}`}
                >
                  <Icon
                  ...
                  >
                    <strong>Action Required:</strong> Collective requested a
                    crop status update. Please select current growth stage
                    below.
                  </p>
                </div>
              ) : (
                <div
                  className={`rounded-xl p-3 border flex items-start gap-2 ${isDark ? "bg-blue-950/30 border-blue-500/20" : "bg-blue-50 border-blue-200"}`}
                >
                  <Icon
                    icon="ph:info-fill"
                  ...
                  >
                    Sending an update notifies the collective about your crop
                    growth stage.
                  </p>
                </div>
              )}
              <div>
                <label
                  className={`text-xs font-semibold block mb-2 ${isDark ? "text-slate-300" : "text-slate-700"}`}
                >
                  Growth Stage *
                </label>
              ...
              <div>
                <label
                  className={`text-xs font-semibold block mb-1.5 ${isDark ? "text-slate-300" : "text-slate-700"}`}
                >
                  Note (optional)
                </label>
                <textarea
                  ...
                  placeholder="e.g. Crop is growing well..."
                  rows={3}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setUpdateForm((p) => ({ ...p, imgUrl: e.target.value }))
                    }
                    placeholder="https://..."
            ...
            >
              {historyLoading ? (
                <div className="flex items-center justify-center h-32">
                  <Icon
                    icon="svg-spinners:12-dots-scale-rotate"
                    className={`w-8 h-8 ${isDark ? "text-emerald-400" : "text-emerald-500"}`}
                  />
                </div>
                ...
                <div className="text-center py-10">
                  <Icon
                    ...
                    className={`w-10 h-10 mx-auto mb-2 ${isDark ? "text-slate-600" : "text-slate-300"}`}
                  />
                  <p
                    className={`text-sm font-semibold ${isDark ? "text-slate-500" : "text-slate-400"}`}
                  >
                    No history yet
                  </p>
                </div>
                  ...
                  <div className="space-y-4 pl-10">
                    {statusHistory.map((h, i) => (
                      <div key={i} className="relative">
                        <div
                          className={`absolute -left-[26px] w-4 h-4 rounded-full border-2 flex items-center justify-center ${isDark ? "bg-slate-900 border-emerald-500" : "bg-white border-emerald-400"}`}
                        >
                          <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                        </div>
                        <div
                          className={`rounded-xl p-3 border ${isDark ? "bg-slate-800/50 border-slate-700/50" : "bg-slate-50 border-slate-200"}`}
                        >
...
const CropManagement = () => {
  ...
  useEffect(() => {
    if (filteredCrops.length > 0) {
      if (
        !selectedCrop ||
        !filteredCrops.some((c) => c._id === selectedCrop._id)
      ) {
  ...
  return (
    ...
    >
        ...
        {view === "list" ? (
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
              <div>
                ...
                >
                  Manage your planted crops and send growth updates to
                  collectives
                </p>
              </div>
            ...
            {loading ? (
              ...
              <div className="relative flex justify-end min-h-[calc(100vh-200px)] w-full">
                ...
                <div
                  className="w-full lg:w-auto lg:absolute lg:top-0 lg:left-0 lg:bottom-0 lg:right-[calc(38%+1.25rem)] lg:overflow-y-auto lg:pr-1"
                  style={{
                    scrollbarWidth: "thin",
          ...
          >
            ...
            >
              ...
              <p
                className={`text-xs sm:text-sm mb-6 ${isDark ? "text-slate-400" : "text-slate-500"}`}
              >
                {editingId
                  ? "Update your crop yield and planting date."
                  : "Select a crop from directory and enter planting info."}
              </p>
              ...
              <form onSubmit={handleSave} className="space-y-5">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm((p) => ({ ...p, plantedDate: e.target.value }))
                    }
                    className={`w-full px-4 py-3 rounded-xl border text-sm outline-none transition-all ${isDark ? "bg-slate-800/50 border-slate-700 text-white focus:border-emerald-500" : "bg-slate-50 border-slate-200 text-slate-900 focus:border-emerald-500"}`}
                  />
                </div>
                <div className="flex gap-3 pt-4 border-t border-slate-200 dark:border-slate-800">
                  <button
                    type="button"
                    onClick={() => setView("list")}
                    className={`flex-1 py-3 rounded-xl text-sm font-semibold border cursor-pointer transition-colors ${isDark ? "border-slate-700 text-slate-300 hover:bg-slate-800" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}
                  >
                    Cancel
                  </button>
                  <button
      ...
      <ConfirmModal
        ...
        title="Delete Crop?"
        description={`Are you sure you want to delete ${cropToDelete?.crop?.name || "this crop"}? This action cannot be undone.`}
