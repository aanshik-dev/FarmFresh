...
const CollectiveBrowse = () => {
  ...
  const myCropIds = farmerCropsData
    .filter((c) => c.status !== "out_of_season")
    .map((c) => c.id.toLowerCase());
  const myCropsLinked = farmerCropsData
    .filter((c) => c.collective)
    .map((c) => c.name.toLowerCase());

  const filtered = collectivesList.filter(
    (c) =>
  ...
  const getMemberStatus = (collectiveId) =>
    memberships.find((m) => m.collectiveId === collectiveId);
  ...
  return (
    ...
    >
      <div className="mb-6">
        ...
        >
          Browse and join collectives for your crops
        </p>
      </div>
      ...
      {memberships.length > 0 && (
        <div
          className={`rounded-2xl border p-5 mb-6 ${isDark ? "bg-slate-900/60 border-slate-800" : "bg-white border-slate-200"}`}
        >
          <h2
        ...
        <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-5">
          {filtered.map((coll) => {
            ...
            return (
              ...
              >
                {/* Header */}
                <div className="relative h-24 bg-linear-to-br from-emerald-600 to-teal-700 flex items-end p-4">
                  {coll.isVerified && (
                ...
                <div className="p-5">
                  ...
                  <p
                    className={`text-xs mb-3 flex items-center gap-1 ${isDark ? "text-slate-400" : "text-slate-500"}`}
                  >
                    <Icon icon="ph:map-pin-fill" className="w-3 h-3" />
                    {coll.address}
                  </p>
                  ...
                  <div className="flex items-center gap-1.5 mb-3">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Icon
                        key={i}
                        icon={
                          i < Math.floor(coll.rating)
                            ? "ph:star-fill"
                            : "ph:star"
                        }
                        className={`w-3.5 h-3.5 ${i < Math.floor(coll.rating) ? "text-amber-400" : isDark ? "text-slate-600" : "text-slate-300"}`}
                      />
                    ))}
                    <span
                      className={`text-xs ${isDark ? "text-slate-400" : "text-slate-500"}`}
                    >
                      {coll.rating} ({coll.reviews})
                    </span>
                  </div>
                    ...
                    {[
                      {
                        ...
                        val: `${coll.farmerGroups} Groups`,
                      },
                      {
                        icon: "ph:person-fill",
                        val: `${coll.workers} Workers`,
                      },
                      { icon: "ph:package-fill", val: coll.totalHarvest },
                    ].map((s) => (
                      <div
                        key={s.val}
                        className={`flex-1 flex items-center gap-1 text-[10px] px-2 py-1.5 rounded-lg ${isDark ? "bg-slate-800 text-slate-400" : "bg-slate-50 text-slate-500"}`}
                      >
                  ...
                  <div className="flex flex-wrap gap-1 mb-4">
                    {coll.crops.map((cr) => (
                      <span
                        key={cr.name}
                        className={`text-[10px] px-2 py-0.5 rounded-full border ${isDark ? "border-slate-700 text-slate-400" : "border-slate-200 text-slate-500"}`}
                      >
                  ...
                  {memberStatus ? (
                    <div
                      className={`flex items-center gap-2 py-2 rounded-xl text-xs font-medium justify-center ${memberStatus.status === "approved" ? (isDark ? "bg-emerald-500/10 text-emerald-400" : "bg-emerald-50 text-emerald-700") : isDark ? "bg-amber-500/10 text-amber-400" : "bg-amber-50 text-amber-700"}`}
                    >
                      <Icon
                        icon={
                    ...
                    <button
                      ...
                      className="w-full py-2 rounded-xl text-xs font-semibold bg-linear-to-r from-emerald-500 to-emerald-600 text-white cursor-pointer hover:from-emerald-400 hover:to-emerald-500 transition-all"
                    >
                      <Icon
        ...
        {selectedCollective && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            >
              ...
              <div
                className={`flex items-center gap-3 p-3 rounded-xl mb-4 ${isDark ? "bg-slate-800/50" : "bg-slate-50"}`}
              >
                <img
                  src={selectedCollective.profilePhoto}
                  alt=""
              ...
              <div className="mb-4">
                ...
                >
                  Which crop for this membership? *
                </label>
                <div className="flex flex-wrap gap-2">
                  {farmerCropsData
                    .filter(
                      (c) => c.status !== "out_of_season" && !c.collective,
                    )
                    .map((crop) => {
                      const isHandled = selectedCollective.crops.some(
              ...
              <div className="mb-5">
                ...
                <textarea
                  ...
                  placeholder="Any special details for the collective…"
                  rows={2}
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className={`flex-1 py-2.5 rounded-xl text-sm bg-linear-to-r from-emerald-500 to-emerald-600 text-white font-semibold cursor-pointer transition-all flex items-center justify-center gap-2 ${requesting || !membershipCrop ? "opacity-60 cursor-not-allowed" : ""}`}
                >
                  {requesting && (
