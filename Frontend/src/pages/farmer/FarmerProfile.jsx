...
const FarmerProfile = () => {
  ...
  const [leadFarmerName, setLeadFarmerName] = useState(user?.leadFarmerName || "");
  const [numberOfFarmers, setNumberOfFarmers] = useState(user?.numberOfFarmers || "");
  ...
  const handleSave = async () => {
    ...
    updateUser({
      name, phone, description, leadFarmerName, groupName,
      numberOfFarmers: parseInt(numberOfFarmers) || numberOfFarmers,
      crops: selectedCrops, address, village, district, state, pincode,
  ...
  const handleSubmitReview = () => {
    ...
    setRating(0); setReviewText(""); setSelectedCollective("");
  };

  const fullAddress = [user?.village, user?.district, user?.state, user?.pincode].filter(Boolean).join(", ");
            ...
            <div className="flex items-end justify-between -mt-10 mb-4 flex-wrap gap-3">
              <div className={`w-20 h-20 rounded-2xl border-4 overflow-hidden flex items-center justify-center ${isDark ? "border-slate-900 bg-emerald-800" : "border-white bg-emerald-100"}`}>
                {user?.profilePhoto
                  ...
                  : <Icon icon="ph:plant-fill" className={`w-10 h-10 ${isDark ? "text-emerald-300" : "text-emerald-600"}`} />
                }
              </div>
            ...
            {editing ? (
              <div className="space-y-4">
                <div className="grid sm:grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <label className={`block text-sm font-medium ${isDark ? "text-slate-300" : "text-slate-700"}`}>Group Name</label>
                    <input className={iClass} value={groupName} onChange={e => setGroupName(e.target.value)} placeholder="Group name" />
                  </div>
                  <div className="space-y-1.5">
                    <label className={`block text-sm font-medium ${isDark ? "text-slate-300" : "text-slate-700"}`}>Lead Farmer Name</label>
                    <input className={iClass} value={leadFarmerName} onChange={e => setLeadFarmerName(e.target.value)} placeholder="Your name" />
                  </div>
                  <div className="space-y-1.5">
                  ...
                  <div className="space-y-1.5">
                    <label className={`block text-sm font-medium ${isDark ? "text-slate-300" : "text-slate-700"}`}>No. of Farmers</label>
                    <input className={iClass} type="number" value={numberOfFarmers} onChange={e => setNumberOfFarmers(e.target.value)} placeholder="12" />
                  </div>
                </div>
                <div className="space-y-1.5">
                  <label className={`block text-sm font-medium ${isDark ? "text-slate-300" : "text-slate-700"}`}>Description</label>
                  <textarea value={description} onChange={e => setDescription(e.target.value)} rows={3} placeholder="About your farmer group…" className={`${iClass} resize-none`} />
                </div>
                ...
                <div className={`rounded-xl border p-4 space-y-3 ${isDark ? "border-slate-700 bg-slate-800/30" : "border-slate-200 bg-slate-50"}`}>
                  ...
                  <div className="grid sm:grid-cols-2 gap-3">
                    {[
                      { label: "Village / Town", val: village, set: setVillage, ph: "e.g. Triyuginarayan" },
                ...
                <div className="space-y-2">
                  <p className={`text-sm font-medium ${isDark ? "text-slate-300" : "text-slate-700"}`}>Crops Grown</p>
              ...
              <div className="space-y-3">
                <p className={`text-sm leading-relaxed ${isDark ? "text-slate-300" : "text-slate-600"}`}>
                  {user?.description || <span className={`italic ${isDark ? "text-slate-500" : "text-slate-400"}`}>No description added yet. Click "Edit Profile" to add one.</span>}
                </p>
                <div className="flex flex-wrap gap-3 text-sm">
                  {[
                    user?.phone && { icon: "ph:phone-fill", val: user.phone },
                    fullAddress && { icon: "ph:map-pin-fill", val: fullAddress },
                    user?.email && { icon: "ph:envelope-fill", val: user.email },
                ...
                {(selectedCrops.length > 0 || (user?.crops?.length > 0)) && (
                  <div className="flex flex-wrap gap-2 pt-1">
                    {(user?.crops || selectedCrops).map(c => (
                      <span key={c} className={`px-2.5 py-1 rounded-xl text-xs font-medium ${isDark ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-emerald-50 text-emerald-700 border border-emerald-200"}`}>{c}</span>
                    ))}
                  </div>
                )}
              </div>
        ...
        <div className={`rounded-2xl border p-5 ${isDark ? "bg-slate-900/60 border-slate-800" : "bg-white border-slate-200"}`}>
          ...
          <div className="space-y-4">
            <div>
              ...
              <select value={selectedCollective} onChange={e => setSelectedCollective(e.target.value)}
