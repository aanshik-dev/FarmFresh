...
const SettingsPage = () => {
  ...
  // Unified notifications (just for UI mock currently)
  ...
  const handleChangePass = async () => {
    ...
    setSaving(true);
    // TODO: Connect this to actual change password API
    await new Promise(r => setTimeout(r, 800));
    toast.success("Password changed!"); 
    setOldPass(""); setNewPass(""); setConfirmPass(""); setSaving(false);
  };
  ...
  const accountInfo = [
    ...
    { label: "Role", value: isFarmer ? "Farmer Group" : "Collective", icon: "ph:identification-badge-fill" },
    { label: "Name", value: user?.name, icon: isFarmer ? "ph:users-three-fill" : "ph:buildings-fill" },
  ];
    ...
    <div className={`min-h-screen p-5 sm:p-7 ${isDark ? "bg-slate-950" : "bg-slate-50"}`}>
      <div className="max-w-2xl mx-auto space-y-5">
        ...
        <div className={`rounded-2xl border p-5 ${isDark ? "bg-red-950/20 border-red-900/30" : "bg-red-50 border-red-200"}`}>
          ...
          <p className={`text-sm mb-4 ${isDark ? "text-red-400/70" : "text-red-600"}`}>
            Permanently deactivate your account. You will not be able to log in again.
          </p>
          <Button variant="danger" onClick={() => setShowDeleteModal(true)} icon="ph:trash-fill">Deactivate Account</Button>
        </div>
      </div>
      <ConfirmModal 
