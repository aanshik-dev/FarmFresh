...
const CollectiveDashboard = () => {
      ...
      if (Array.isArray(s.items)) {
        s.items.forEach((item) => {
          if (item.status === "COLLECTED" && item.paymentStatus === "PENDING") {
            ...
            const name = fg?.groupName || fg?.leadFarmer || item.farmerName || "Farmer Group";
  ...
  const activePieData =
    pieMode === "revenue" ? revenueDistributedShareData : inventoryQuantityShareData;
  ...
  const collectiveTitle = user?.collectiveName || user?.name || "Garima Collective";
  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-teal-700 via-emerald-700 to-green-800 p-6 sm:p-8 text-white shadow-2xl shadow-emerald-950/40">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2 max-w-xl">
            ...
            <p className="text-sm text-emerald-100 font-medium">
              Coordinate pickup schedules across zones, manage member farmer groups, and monitor crop logistics in real-time.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              <Icon icon="ph:calendar-plus-bold" className="w-4 h-4 text-emerald-700" />
              Schedule Pickup
            </button>
            <button
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          label="Managed Crops"
          valueObj={getFormattedNumberParts(stats.totalCrops || inventoryList.length || 0)}
          icon="ph:plant-fill"
        ...
        <StatCard
          label="Live Pickups"
          valueObj={getFormattedNumberParts(stats.upcomingPickups || upcomingSchedules.length || 0)}
          icon="ph:truck-fill"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        >
          ...
          <div>
            <p className={`text-xs font-bold ${isDark ? "text-white" : "text-slate-900"}`}>Crop Inventory</p>
            <p className="text-[10px] text-slate-400">Set prices & view pools</p>
          </div>
        </div>
        ...
        >
          ...
          <div>
            <p className={`text-xs font-bold ${isDark ? "text-white" : "text-slate-900"}`}>Driver Fleet</p>
            <p className="text-[10px] text-slate-400">Assign & manage drivers</p>
          </div>
        </div>
        ...
        >
          ...
          <div>
            <p className={`text-xs font-bold ${isDark ? "text-white" : "text-slate-900"}`}>Payout History</p>
            <p className="text-[10px] text-slate-400">Farmer payments & receipts</p>
          </div>
        </div>
        ...
        >
          ...
          <div>
            <p className={`text-xs font-bold ${isDark ? "text-white" : "text-slate-900"}`}>Zone Logistics</p>
            <p className="text-[10px] text-slate-400">Configure altitude zones</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        >
          ...
          {upcomingSchedules.length === 0 ? (
            ...
            >
              <Icon icon="ph:truck-fill" className="w-9 h-9 mx-auto mb-2 opacity-30 text-emerald-500" />
              <p className="text-sm font-semibold">No active pickup runs pending.</p>
              <button
                onClick={() => navigate("/dashboard/collective/schedules")}
              ...
              {upcomingSchedules.slice(0, 3).map((sch) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3 my-1.5">
                    ...
                    >
                      <Icon icon="ph:steering-wheel-bold" className="w-4 h-4 text-emerald-400" />
                    </div>
        ...
        >
          ...
          {pendingDuesList.length === 0 ? (
            ...
            >
              <Icon icon="ph:check-circle-fill" className="w-9 h-9 mx-auto mb-2 opacity-40 text-emerald-400" />
              <p className="text-sm font-semibold text-emerald-400">All farmer payouts settled!</p>
              <p className="text-xs text-slate-400 mt-1">No outstanding dues pending across farmer groups.</p>
            </div>
          ) : (
            <div className="space-y-3">
        ...
        >
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
            <div>
              ...
              >
                <Icon
                  icon={pieMode === "revenue" ? "ph:currency-inr-bold" : "ph:package-fill"}
            ...
            <div
              className={`flex items-center gap-1 p-1 rounded-xl border shrink-0 ${
                isDark ? "bg-slate-800 border-slate-700/60" : "bg-slate-100 border-slate-200"
              }`}
            >
              <button
                ...
                <PieChart>
                  ...
                  <Tooltip
                    ...
                    formatter={(val) => [
                      ...
                      pieMode === "revenue" ? "Settled Revenue" : "Inventory Quantity",
                    ]}
                  />
                  <Legend
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={monthlyHarvestData}>
                ...
                <Tooltip
                  {...chartTheme.tooltip}
                  formatter={(val) => [`₹${Number(val).toLocaleString("en-IN")}`, "Collection Value"]}
                />
                <Line
                  type="monotone"
                  dataKey="amount"
